#!/usr/bin/env python3
"""Revalidate every preflight-bound R04 input and infrastructure artifact."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-r04-preservation-v1"
...
def load_helper() -> Any:
    path = Path(__file__).with_name("w1_cp05_r04_preflight.py")
    spec = importlib.util.spec_from_file_location("w1_cp05_r04_preflight_import", path)
    require(spec is not None and spec.loader is not None, "cannot import R04 preflight helper")
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    require(preflight.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r04-preflight-v1"
            and preflight.get("status") == "pass", "R04 preflight not PASS")
    ...
    if mismatches: raise ValueError("R04 protected input/infrastructure changed")
    return report
