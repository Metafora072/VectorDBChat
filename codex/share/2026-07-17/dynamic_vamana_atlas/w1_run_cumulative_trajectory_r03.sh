#!/usr/bin/env bash
# R03-only shared CP00 -> CP01 -> CP05 state machine for DGAI and OdinANN.
set -euo pipefail
...
[[ ${W1_CP05_R03_CUMULATIVE_AUTHORIZED:-0} == 1 && ${W1_GLOBAL_LOCK_HELD:-0} == 1 ]] || {
  echo 'R03 cumulative trajectory/global-lock capability absent' >&2; exit 64;
}
...
if [[ ${arg[mode]} == formal ]]; then
  ...
  [[ ${arg[run-name]} == pilot3_sift10m_w1_cp05_trajectory_r03 && ${arg[attempt]} == trajectory-cp05-03 ]] || {
    echo 'R03 formal run/attempt identity mismatch' >&2; exit 1;
  }
else
  [[ ${arg[cp01-count]} == 16 && ${arg[cp05-count]} == 64 ]] || {
    echo 'sequential replay counts must be 16 then 64 delta' >&2; exit 1;
  }
  [[ ${arg[run-name]} == pilot3_w1_cp05_trajectory_replay_r03 && ${arg[attempt]} == sequential-cp80-03 ]] || {
    echo 'R03 replay run/attempt identity mismatch' >&2; exit 1;
  }
fi
...
[[ -f $evidence && -f $r02/w1_cumulative_stage_worker.sh && -x $new/w1_run_query_scope.sh ]] || { echo 'cumulative helper missing' >&2; exit 1; }
...
if [[ ${arg[mode]} == formal ]]; then
  result=$(realpath -m "$root/results/pilot3_sift10m_w1_cp05_trajectory_r03/${arg[system]}/${arg[attempt]}")
else
  result=$(realpath -m "$root/results/pilot3_sift10m_w1_cp05_trajectory_r03/replay/${arg[system]}/${arg[attempt]}")
fi
expected_work="$root/formal/${arg[run-name]}/${arg[system]}/${arg[attempt]}"
if [[ ${arg[mode]} == formal ]]; then expected_result="$root/results/pilot3_sift10m_w1_cp05_trajectory_r03/${arg[system]}/${arg[attempt]}"
else expected_result="$root/results/pilot3_sift10m_w1_cp05_trajectory_r03/replay/${arg[system]}/${arg[attempt]}"; fi
...
[[ $base == "$expected_base" ]] || { echo "R03 base identity mismatch: $base" >&2; exit 1; }
...
if [[ ${arg[mode]} == replay ]]; then
  clone_helper="$new/w1_clone_replay_base_r03.sh"
else
...
query_checkpoint() {
  ...
  for l in "${query_ls[@]}"; do
    for repetition in 1 2 3; do
      ...
      local unit="dv-w1-cum-r03-${arg[mode]}-${system_lower}-${checkpoint}-l${l}-r${repetition}"
      "$new/w1_run_query_scope.sh" --unit "$unit" --system "${arg[system]}" \
...
inaccessible_canary() {
  ...
  local unit="dv-w1-cum-r03-${arg[mode]}-${system_lower}-${checkpoint}-input-canary"
...
run_stage() {
  ...
  local stage_result="$result/stages/$checkpoint" resource="$result/${checkpoint}_stage_resources.json"
  local unit="dv-w1-cum-r03-${arg[mode]}-${system_lower}-${checkpoint}-update"
  ...
  systemd-run --wait --collect --pipe --unit "$unit" --uid ubuntu --property=Type=exec \
    ...
    numactl --physcpubind=0-23 --membind=0 \
      ...
      "$r02/w1_cumulative_stage_worker.sh" --mode "${arg[mode]}" --system "${arg[system]}" --stage "$checkpoint" \
