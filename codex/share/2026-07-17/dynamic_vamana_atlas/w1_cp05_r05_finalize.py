#!/usr/bin/env python3
"""Tamper-evident finalizer for CP05 cumulative trajectory R05 outputs."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-trajectory-r05-summary-v1"
RUN = "pilot3_sift10m_w1_cp05_trajectory_r05"
REPLAY_RUN = "pilot3_w1_cp05_trajectory_replay_r05"
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    old = module("w1_cumulative_finalize_r05_import", "w1_cumulative_finalize.py")
    prehelper = module("w1_cp05_r05_preflight_finalize_import", "w1_cp05_r05_preflight.py")
    ...
    require(result == root / f"results/{RUN}" and formal_root == root / f"formal/{RUN}",
            "R05 finalizer capability path mismatch")
    require(replay_result in (result / "replay", root / f"results/{REPLAY_RUN}"),
            "R05 replay result root mismatch")
    expected_report = Path(__file__).parent.parent / "dynamic_vamana_w1_cp05_cumulative_trajectory_r05_results_0717.md"
    require(args.output_report.resolve(strict=False) == expected_report.resolve(strict=False),
            "R05 report output path mismatch")
    ...
    require(preflight.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r05-preflight-v1"
            and preflight.get("status") == "pass", "R05 preflight invalid")
    preservation_path = args.preservation.resolve(strict=True); preservation = load(preservation_path)
    require(preservation.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r05-preservation-v1"
            and preservation.get("status") == "pass" and not preservation.get("mismatches"),
            "R05 preservation invalid")
    ...
    for system in ("DGAI", "OdinANN"):
        replay_attempt = replay_result / system / "sequential-cp80-05"
        replay_work = replay_work_root / system / "sequential-cp80-05"
        ...
        attempt = result / system / "trajectory-cp05-05"; work = formal_root / system / "trajectory-cp05-05"
    ...
    disk_path = result / "DiskANN/stale-cp05-05/stale_control.json"; disk = old.require_pass(disk_path)
    ...
    summary = {"schema": SCHEMA, "status": "pass",
        "decision_boundary": "CP05 cumulative R05 only; CP10/CP20 remain HOLD",
    ...
    lines = ["# Dynamic Vamana W1 CP05 累计 Trajectory R05 实验结果", "", "## 裁决", "",
