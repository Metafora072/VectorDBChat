#!/usr/bin/env python3
"""Fail-closed preflight for CP05 cumulative trajectory R05 recovery."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-r05-preflight-v1"
...
R05_REPLAY_RUN = "pilot3_w1_cp05_trajectory_replay_r05"
...
def active_w1_processes() -> list[dict[str, Any]]:
    ...
    rows = []
    executable_tokens = (
        "run_w1_cp05_cumulative_trajectory_r05.sh",
        "start_w1_cp05_cumulative_r05_tmux.sh",
        "w1_run_cumulative_trajectory_r05.sh",
        "w1_cumulative_stage_worker_r05.sh",
        "w1_run_query_scope.sh",
        "w1_run_diskann_cp05_stale_r05.sh",
    )
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    r04_result = args.r04_terminal_result.resolve(strict=True)
    r04_replay_formal = args.r04_terminal_replay_formal.resolve(strict=True)
    ...
    result = args.r05_result_root.resolve(strict=False); formal = args.r05_formal_root.resolve(strict=False)
    replay_formal = args.r05_replay_formal_root.resolve(strict=False)
    ...
    require(r04_result == root / f"results/{R04_RUN}"
            and r04_replay_formal == root / "formal/pilot3_w1_cp05_trajectory_replay_r04",
            "R04 terminal capability path mismatch")
    require(result == root / f"results/{R05_RUN}" and formal == root / f"formal/{R05_RUN}"
            and replay_formal == root / f"formal/{R05_REPLAY_RUN}", "R05 capability path mismatch")
    require(output == result / "preflight/execution_preflight.json", "R05 preflight output path mismatch")
    require(delta == root / "datasets/sift10m/w1_trajectory/execution_deltas_r03",
            "R05 must reuse the accepted R03 delta root")
    require(replay_inputs == r03_result / "replay/inputs",
            "R05 must reuse the accepted R03 replay-input root")
    ...
    r04_execution_path = r04_result / "execution_manifest.json"; r04_execution = load(r04_execution_path)
    require((r04_execution.get("schema"), r04_execution.get("status"),
             r04_execution.get("stopped_phase"), r04_execution.get("exit_code"))
            == ("dynamic-vamana-w1-cp05-cumulative-r04-execution-v1",
                "stopped_failed", "replay_DGAI", 1), "R04 terminal identity mismatch")
    require(r04_execution.get("attempts", {}).get("DGAI_replay") == "sequential-cp80-04",
            "R04 replay attempt identity mismatch")
    r04_preservation_path = r04_result / "preflight/preservation_after_stop.json"
    r04_preservation = load(r04_preservation_path)
    require(r04_preservation.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r04-preservation-v1"
            and r04_preservation.get("status") == "pass" and not r04_preservation.get("mismatches"),
            "R04 stop-time preservation is not PASS")
    r04_query_gate_path = r04_result / "replay/DGAI/sequential-cp80-04/queries/cp00/query_gate.json"
    r04_query_gate = load(r04_query_gate_path)
    require(r04_query_gate.get("schema") == "dynamic-vamana-w1-query-identity-v2"
            and r04_query_gate.get("status") == "pass" and r04_query_gate.get("checkpoint") == "cp00",
            "R04 DGAI CP00 query gate is not PASS")
    r04_canary_path = (r04_result / "replay/DGAI/sequential-cp80-04/stages/cp01"
                       / "input_canary/canary.json")
    r04_canary = load(r04_canary_path)
    require(r04_canary.get("schema") == "dynamic-vamana-w1-r04-input-canary-v1"
            and r04_canary.get("status") == "pass" and r04_canary.get("uid") == 1000
            and r04_canary.get("allowed_readable") is True
            and r04_canary.get("denied")
            and all(row.get("open_refused") is True for row in r04_canary.get("denied", [])),
            "R04 stage-local input canary is not PASS")
    r04_forbidden = (list(r04_result.glob("**/STAGE_WORKER_OK"))
                     + list(r04_result.glob("**/stage_evidence.json"))
                     + list(r04_result.glob("**/*checkpoint_evidence.json"))
                     + list(r04_result.glob("**/CUMULATIVE_TRAJECTORY_OK"))
                     + list(r04_result.glob("**/markers.jsonl")))
    require(not r04_forbidden and not (r04_result / "replay/OdinANN").exists()
            and not (r04_result / "DGAI").exists() and not (r04_result / "OdinANN").exists()
            and not (r04_result / "DiskANN").exists()
            and not (root / f"formal/{R04_RUN}").exists(),
            "R04 advanced beyond the accepted pre-update boundary")
    require((r04_replay_formal / "DGAI/sequential-cp80-04/clone_manifest.json").is_file(),
            "R04 terminal clone evidence absent")
    ...
    require(canary_tests.get("schema") == "dynamic-vamana-w1-r04-minimal-input-canary-tests-v1"
            ...
            "R05 minimal input-canary tests not PASS")
    ...
    require(helper_record.get("realpath") == str(canary_helper)
            ...
            "R05 input-canary helper identity mismatch")
    ...
    # R05 is fresh: before activation it may contain only recovery-gate evidence.
    if result.exists():
        require({entry.name for entry in result.iterdir()} <= {"preflight"},
                "R05 result contains attempt output")
        require(not any(result.glob("DGAI/**")) and not any(result.glob("OdinANN/**"))
                and not any(result.glob("DiskANN/**")), "R05 result attempt not fresh")
    require(not formal.exists() and not replay_formal.exists(), "R05 formal/replay clone target not fresh")
    require(not args.report.exists(), "R05 final report target is not fresh")
    ...
    report = {"schema": SCHEMA, "status": "pass", "run": R05_RUN,
        "attempts": {"DGAI": "trajectory-cp05-05", "OdinANN": "trajectory-cp05-05",
                     "DiskANN": "stale-cp05-05", "replay": "sequential-cp80-05"},
        "terminal_attempt": {"R03": {"execution_manifest": identity(r03_execution_path),
            ...
            "R04": {"execution_manifest": identity(r04_execution_path),
                "status": "stopped_failed", "stopped_phase": "replay_DGAI", "exit_code": 1,
                "stopped_before_update_api": True, "no_cp01_cp05_odin_formal_diskann": True,
                "cp00_query_gate": identity(r04_query_gate_path),
                "stage_local_canary": identity(r04_canary_path),
                "preservation_after_stop": identity(r04_preservation_path)}},
...
def self_test(args: argparse.Namespace) -> None:
    ...
    write_new(args.output, {"schema": "dynamic-vamana-w1-cp05-r05-preflight-self-test-v1",
...
def parser() -> argparse.ArgumentParser:
    ...
    for name in ("root", "artifact-manifest", "r03-result", "r03-formal",
                 ...
                 "r04-terminal-result", "r04-terminal-replay-formal",
                 "replay-base-root", "static-smoke", "static-smoke-revalidation",
                 "query-scope-tests", "input-canary-tests", "r05-result-root",
                 "r05-formal-root", "r05-replay-formal-root", "report", "output"):
