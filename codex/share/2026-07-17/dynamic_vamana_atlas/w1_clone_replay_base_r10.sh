#!/usr/bin/env bash
# Clone one exact R10 immutable replay base into its exact mutable attempt.
set -euo pipefail

[[ ${W1_CP05_R10_CUMULATIVE_AUTHORIZED:-0} == 1 && ${W1_FORMAL_PATH_AUTHORIZED:-0} == 1 ]] || { echo 'R10 replay clone gate not granted' >&2; exit 64; }
...
for name in W1_ALLOWED_CLONE_TARGET W1_ALLOWED_CLONE_SYSTEM W1_ALLOWED_CLONE_RUN W1_ALLOWED_CLONE_ATTEMPT; do
  [[ -n ${!name:-} ]] || { echo "R10 replay clone capability missing: $name" >&2; exit 2; }
done
expected_run=pilot3_w1_cp05_trajectory_replay_r10
expected_attempt=sequential-cp80-10
...
[[ $W1_ALLOWED_CLONE_RUN == "$expected_run" && $W1_ALLOWED_CLONE_ATTEMPT == "$expected_attempt" \
  ...
  echo 'R10 replay clone identity mismatch' >&2; exit 2;
}
[[ -f "$(dirname "$base")/IMMUTABLE_REPLAY_BASE_OK" ]] || { echo 'R10 immutable replay marker absent' >&2; exit 1; }
