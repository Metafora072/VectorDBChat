...
def main():
 ...
 audited=[{'entry':async_entry,'ledger':'async'}]+[{'entry':x,'ledger':'posix'} for x in ('write','pwrite','pwrite64','writev','pwritev','sendfile','fdatasync/fsync')]+[{'entry':'insert_rmw_role','ledger':'logical'}]
 ...
 source_complete=(observed.get(('async',async_entry),0)>0 and bool(roles)
                  and (a.system!='OdinANN' or observed.get(('posix','sendfile'),0)>0)
                  and all(x['ledger'] in ('async','posix','logical') for x in audited))
 ...
 br=build['systems'][a.system];build_ok=build.get('status')=='pass' and br.get('binary_is_independent') is True and build.get('schema')=='dynamic-vamana-write-attribution-m0-build-v5'
