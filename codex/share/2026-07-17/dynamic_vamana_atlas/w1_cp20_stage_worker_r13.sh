...
usage() {
  cat >&2 <<'EOF'
usage: w1_cp20_stage_worker_r13.sh
  --mode formal --system DGAI|OdinANN --stage cp20
...
[[ ${arg[mode]} == formal ]] || usage
[[ ${arg[system]} == DGAI || ${arg[system]} == OdinANN ]] || usage
[[ ${arg[stage]} == cp20 ]] || usage
...
if [[ -e $result ]]; then
  [[ -d $result && -d $result/input_canary && ! -L $result/input_canary \
    ...
    echo 'stage result lacks the exact R13 input-canary evidence' >&2; exit 1;
  }
  ...
  [[ ${existing[*]} == 'input_canary input_canary/canary.json input_canary/canary.log' ]] || {
    echo 'stage result contains state beyond the R13 input canary' >&2; exit 1;
  }
else
...
# This precheck is intentionally inside the update scope.  It proves that R13
# receives only master records [800000:1600000], never a cumulative prefix.
python3 - "$trace" "$delta_manifest" "${arg[expected-count]}" "${arg[delta-start]}" "$full" "$before_tags" "$after_tags" "$probes" "$probe_spec" "$local_spec" "$global_spec" "$result/worker_identity.json" "$scope" "${arg[mode]}" "${arg[system]}" "${arg[stage]}" "$attempt" "$new/w1_cp20_stage_worker_r13.sh" "${arg[forbidden-paths]}" <<'PY'
...
if (mode, stage, delta_start, count) != ("formal", "cp20", 800_000, 800_000):
    raise SystemExit("R13 worker must consume exactly master[800000:1600000]")
if "master" in Path(trace).name.lower() or "replace_cp20" in Path(trace).name.lower():
    raise SystemExit("R13 worker refuses master/cumulative CP20 trace pathnames")
