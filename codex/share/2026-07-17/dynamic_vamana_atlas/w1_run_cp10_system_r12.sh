#!/usr/bin/env bash
# Run one formal CP05->CP10 continuation from an R10 frozen clone.
set -euo pipefail
export PYTHONDONTWRITEBYTECODE=1
[[ ${W1_CP10_R12_AUTHORIZED:-0} == 1 && ${W1_GLOBAL_LOCK_HELD:-0} == 1 ]] || {
  echo 'R12 authorization/global lock absent' >&2; exit 64;
}
(( EUID == 0 )) || { echo 'R12 system runner must run as root' >&2; exit 1; }
...
run=pilot3_sift10m_w1_cp10_trajectory_r12; attempt=trajectory-cp10-12
work="$root/formal/$run/$system/$attempt"
result="$root/results/$run/$system/$attempt"
input="$root/results/$run/inputs/cp05_to_cp10"
...
before_active="$trajectory/cp05/active_cp05.tags.bin"
after_active="$trajectory/cp10/active_cp10.tags.bin"
trace="$input/delta_cp05_to_cp10.bin"; delta_manifest="$input/delta_manifest.json"
...
global_spec="$trajectory/cp10/visibility_probes.json"
artifact="$old/artifact_rebuild_manifest.json"; evidence="$new/w1_cp10_evidence_r12.py"
...
if d.get('status')!='pass' or d.get('system')!=sys.argv[2] or d.get('checkpoint')!='cp05':
 raise SystemExit('R10 freeze evidence mismatch')
print(Path(d['root_realpath']).resolve())
PY
)
[[ $base == "$expected_base" && ! -e $work && ! -e $result ]] || { echo 'R12 base/target capability mismatch' >&2; exit 1; }
[[ $(findmnt -rn -T "$root" -o MAJ:MIN | awk 'NR==1{print;exit}') == "$device" ]] || { echo 'R12 root is not project NVMe' >&2; exit 1; }
for file in "$driver" "$query_binary" "$full" "$query" "$gt" "$before_active" "$after_active" "$trace" "$delta_manifest" "$local_bin" "$local_spec" "$combined_bin" "$combined_spec" "$global_spec"; do
  [[ -f $file ]] || { echo "missing R12 input: $file" >&2; exit 1; }
done
...
cmp -s "$work/base_content_before.tsv" "$result/base_content_after_clone.tsv" || { echo 'R10 frozen base content changed during clone' >&2; exit 1; }
cmp -s "$work/base_mode_before.tsv" "$result/base_mode_after_clone.tsv" || { echo 'R10 frozen base mode changed during clone' >&2; exit 1; }

stage="$result/stages/cp10"; canary="$stage/input_canary"
...
canary_unit="dv-w1-cp10-r12-${system_lower}-input-canary"
systemd-run --wait --collect --pipe --unit "$canary_unit" --uid ubuntu --property=Type=exec \
  ...
  --property="InaccessiblePaths=$trajectory/cp05/replace_cp05.bin" \
  ...
  python3 "$new/w1_input_canary.py" --allowed "$trace" \
    ...
    --denied "$trajectory/cp05/replace_cp05.bin" --denied "$trajectory/cp10/replace_cp10.bin" \
...
resource="$result/cp10_stage_resources.json"; primer="$result/cp10_stage_io_primer.json"
unit="dv-w1-cp10-r12-${system_lower}-update"
systemd-run --wait --collect --pipe --unit "$unit" --uid ubuntu --property=Type=exec \
  ...
  --property=IOAccounting=yes --property=MemoryMax=32G --property=LimitCORE=0 --property=RuntimeMaxSec=2700 \
  --property="InaccessiblePaths=$trajectory/master_replacements_1600k.bin" \
  --property="InaccessiblePaths=$trajectory/cp05/replace_cp05.bin" \
  --property="InaccessiblePaths=$trajectory/cp10/replace_cp10.bin" \
  ...
  numactl --physcpubind=0-23 --membind=0 timeout --signal=TERM --kill-after=30s 45m \
  ...
  "$new/w1_cp10_stage_worker_r12.sh" --mode formal --system "$system" --stage cp10 \
    --attempt-dir "$work" --full-corpus "$full" --driver "$driver" --trace "$trace" \
    --delta-manifest "$delta_manifest" --expected-count 400000 --delta-start 400000 \
    --forbidden-paths "$trajectory/master_replacements_1600k.bin,$trajectory/cp05/replace_cp05.bin,$trajectory/cp10/replace_cp10.bin" \
    ...
    >>"$result/cp10_controller.log" 2>&1
[[ -f $stage/STAGE_WORKER_OK ]] || { echo 'R12 stage worker marker absent' >&2; exit 1; }
...
python3 "$old/w1_collect_canary.py" --system "$system" --markers "$stage/markers.jsonl" \
  ...
  --logical-replacements 400000 --logical-payload-bytes $((400000 * 128 * 4)) \
...
stage_args=(runuser -u ubuntu -- env PYTHONDONTWRITEBYTECODE=1 python3 "$evidence" stage-evidence
  --mode formal --system "$system" --checkpoint cp10 --attempt-dir "$work" --index-root "$work/index"
  ...
  --combined-probe-spec "$combined_spec" --fresh-result "$stage/fresh.bin" --controller-log "$result/cp10_controller.log"
...
checkpoint="$result/checkpoints/cp10"; qdir="$result/queries/cp10"
install -d -o ubuntu -g ubuntu "$checkpoint" "$qdir"
python3 "$old/w1_file_manifest.py" --root "$work/index" --output "$checkpoint/cp10_state_content_manifest.tsv"
python3 "$r02/w1_mode_manifest.py" write --root "$work/index" --output "$checkpoint/cp10_state_mode_manifest.tsv"
...
runuser -u ubuntu -- env PYTHONDONTWRITEBYTECODE=1 python3 "$evidence" query-gate \
  --mode formal --system "$system" --checkpoint cp10 --result-dir "$qdir" --prefix cp10 \
  --binary "$query_binary" --driver "$driver" --artifact-manifest "$artifact" \
  --index-content-manifest "$checkpoint/cp10_state_content_manifest.tsv" --query "$query" --gt "$gt" \
...
cmp -s "$checkpoint/cp10_state_content_manifest.tsv" "$result/index_content_after_query.tsv" || { echo 'CP10 query changed index content' >&2; exit 1; }
cmp -s "$checkpoint/cp10_state_mode_manifest.tsv" "$result/index_mode_after_query.tsv" || { echo 'CP10 query changed index mode' >&2; exit 1; }
...
runuser -u ubuntu -- env PYTHONDONTWRITEBYTECODE=1 python3 "$evidence" checkpoint \
  ...
  --state-content-manifest "$checkpoint/cp10_state_content_manifest.tsv" \
  --state-mode-manifest "$checkpoint/cp10_state_mode_manifest.tsv" \
...
runuser -u ubuntu -- env PYTHONDONTWRITEBYTECODE=1 python3 "$evidence" freeze --mode formal --system "$system" \
  ...
  --checkpoint-evidence "$checkpoint/cp10_checkpoint_evidence.json" --output-dir "$checkpoint"
[[ -f $work/IMMUTABLE_TRAJECTORY_CP10_OK && -f $checkpoint/cp10_freeze_evidence.json ]] || { echo 'R12 freeze evidence absent' >&2; exit 1; }
