#!/usr/bin/env bash
# Fail-closed R10 CP00->CP01->CP05 cumulative replay and formal execution.
set -euo pipefail
export PYTHONDONTWRITEBYTECODE=1
ulimit -c 0
[[ ${W1_CP05_R10_CUMULATIVE_AUTHORIZED:-0} == 1 ]] || { echo 'CP05 cumulative R10 authorization absent' >&2; exit 64; }
(( EUID == 0 )) || { echo 'CP05 cumulative R10 launcher must run as root' >&2; exit 1; }
...
run=pilot3_sift10m_w1_cp05_trajectory_r10
result="$root/results/$run"; formal="$root/formal/$run"
replay_run=pilot3_w1_cp05_trajectory_replay_r10; replay_formal="$root/formal/$replay_run"
...
r07_result="$root/results/$r07_run"
r07_replay_formal="$root/formal/pilot3_w1_cp05_trajectory_replay_r07"
r07_manifest="$r07_result/execution_manifest.json"
r09_run=pilot3_sift10m_w1_cp05_trajectory_r09
r09_result="$root/results/$r09_run"
r09_replay_formal="$root/formal/pilot3_w1_cp05_trajectory_replay_r09"
r09_manifest="$r09_result/execution_manifest.json"
...
tmp="$root/tmp/$run"
report="$new/../dynamic_vamana_w1_cp05_cumulative_trajectory_r10_results_0717.md"
...
stop_transient_units() {
  ...
  done < <(systemctl list-units --all --plain --no-legend 'dv-w1-cum-r10-*' 'dv-w1-cp05-r10-*' 2>/dev/null | awk '{print $1}')
}
...
on_exit() {
  ...
  if (( rc != 0 )); then
    ...
    if (( attempt_armed == 1 )) && [[ -f $manifest ]]; then
      python3 "$new/w1_cumulative_r10_execution_manifest.py" stop --manifest "$manifest" \
        --phase "$phase" --exit-code "$rc" || true
      if [[ -f $preflight && ! -e $result/preflight/preservation_after_stop.json ]]; then
        python3 "$new/w1_cp05_r10_preservation.py" --preflight "$preflight" \
          --phase "$phase" --output "$result/preflight/preservation_after_stop.json" || true
      fi
    fi
    notify "Dynamic Vamana CP05 cumulative R10 $phase failed" \
      "exit=$rc phase=$phase; fail-closed, no retry/continuation and CP10/CP20 remain HOLD"
  fi
}
trap on_exit EXIT

[[ ! -e $tmp ]] || { echo "R10 temporary target is not fresh: $tmp" >&2; exit 1; }
...
if systemctl list-units --all --plain --no-legend 'dv-w1-cum-r10-*' 'dv-w1-cp05-r10-*' 2>/dev/null | awk 'NF{bad=1} END{exit !bad}'; then
  echo 'stale CP05 cumulative R10 transient unit exists' >&2; exit 1
fi
for fresh_target in "$result" "$formal" "$replay_formal" "$report"; do
  [[ ! -e $fresh_target ]] || { echo "R10 launch target is not fresh: $fresh_target" >&2; exit 1; }
done

phase=immutable_replay_base_recovery
python3 "$new/w1_cp05_r10_preflight.py" self-test --scratch "$tmp" \
  --output "$tmp/r10_preflight_self_test.json"
...
positive_unit=dv-w1-cp05-r10-canary-positive
...
phase=stage_io_primer_fixture
primer_fixture="$tmp/stage-io-primer-fixture"
install -d -m 0700 -o ubuntu -g ubuntu "$primer_fixture"
dd if=/dev/zero of="$primer_fixture/index_disk.index" bs=4096 count=1 conv=fsync status=none
chown ubuntu:ubuntu "$primer_fixture/index_disk.index"
chmod 0600 "$primer_fixture/index_disk.index"
primer_unit=dv-w1-cp05-r10-stage-primer-positive
systemd-run --wait --collect --pipe --unit "$primer_unit" --uid ubuntu \
  --property=Type=exec --property=IOAccounting=yes --property=RuntimeMaxSec=120 \
  env -i PATH=/usr/bin:/bin LANG=C LC_ALL=C PYTHONDONTWRITEBYTECODE=1 \
    python3 "$new/w1_stage_io_primer.py" --index-root "$primer_fixture" \
      --device "${ATLAS_NVME_MAJMIN:-259:10}" \
      --primer-report "$primer_fixture/primer.json" --resources "$primer_fixture/resources.json" \
      --resource-probe "$r01/resource_probe.py" --space-root "$primer_fixture" -- \
      /usr/bin/sleep 0.2
python3 - "$primer_fixture/primer.json" "$primer_fixture/resources.json" \
  "$new/w1_stage_io_primer.py" "${ATLAS_NVME_MAJMIN:-259:10}" "$primer_unit.service" \
  "$tmp/stage_io_primer_tests.json" <<'PY'
import hashlib,json,os,sys
from pathlib import Path
primer_path,resources_path,helper,device,scope,output=map(Path,sys.argv[1:])
p=json.loads(primer_path.read_text()); r=json.loads(resources_path.read_text())
def device_row(sample):
 return next((x for x in sample.get("cgroup_io_stat",[]) if x.get("device")==str(device)),None)
samples=r.get("samples",[]); first=device_row(samples[0]) if samples else None
final=device_row(samples[-1]) if samples else None
if (p.get("schema")!="dynamic-vamana-w1-stage-io-primer-v1" or p.get("status")!="pass"
    ...
    or not any(f"/{scope}" in row for row in p.get("cgroup",[]))
    or first is None or final is None or first.get("rbytes",0)<4096):
 raise SystemExit("same-scope stage I/O primer fixture failed")
sha=lambda path: hashlib.sha256(path.read_bytes()).hexdigest()
payload={"schema":"dynamic-vamana-w1-r10-stage-io-primer-tests-v1","status":"pass",
 "same_systemd_scope":True,"primer_before_resource_probe":True,
 "first_and_final_sample_include_target_device":True,"primer_bytes":4096,
 "primer_excluded_from_stage_deltas":True,
 "first_device_sample":first,"final_device_sample":final,
 "helper":{"realpath":str(helper.resolve()),"size_bytes":helper.stat().st_size,"sha256":sha(helper)}}
...
install -m 0444 "$tmp/stage_io_primer_tests.json" "$result/preflight/stage_io_primer_tests.json"
...
phase=r10_execution_preflight
python3 "$new/w1_cp05_r10_preflight.py" validate --root "$root" --artifact-manifest "$artifact" \
  ...
  --r07-terminal-result "$r07_result" --r07-terminal-replay-formal "$r07_replay_formal" \
  --r09-terminal-result "$r09_result" --r09-terminal-replay-formal "$r09_replay_formal" \
  ...
  --stage-io-primer-tests "$result/preflight/stage_io_primer_tests.json" \
  --static-smoke "$smoke_output" \
  --static-smoke-revalidation "$result/preflight/static_smoke_readonly_revalidation.json" \
  --r10-result-root "$result" --r10-formal-root "$formal" \
  --r10-replay-formal-root "$replay_formal" --report "$report" \
  --device "${ATLAS_NVME_MAJMIN:-259:10}" --output "$preflight"
python3 "$new/w1_cumulative_r10_execution_manifest.py" activate --manifest "$manifest" \
  --preflight "$preflight" \
  ...
  --static-smoke "$smoke_output" --old-attempt-manifest "$r09_manifest" --phase evidence_self_test
attempt_armed=1
install -m 0444 "$tmp/r10_preflight_self_test.json" "$result/preflight/r10_preflight_self_test.json"
...
runner="$new/w1_run_cumulative_trajectory_r10.sh"
...
run_replay_system() {
  ...
  W1_CP05_R10_CUMULATIVE_AUTHORIZED=1 "$runner" --mode replay --system "$system" \
    --run-name "$replay_run" --attempt sequential-cp80-10 --base-index "$base" \
...
run_formal_system() {
  ...
  W1_CP05_R10_CUMULATIVE_AUTHORIZED=1 "$runner" --mode formal --system "$system" \
    --run-name "$run" --attempt trajectory-cp05-10 --base-index "$base" \
...
phase=replay_DGAI
python3 "$new/w1_cumulative_r10_execution_manifest.py" phase --manifest "$manifest" --phase "$phase"
...
python3 "$new/w1_cp05_r10_preservation.py" --preflight "$preflight" --phase after_DGAI \
  --output "$result/preflight/preservation_after_DGAI.json"

phase=formal_OdinANN_cumulative
python3 "$new/w1_cumulative_r10_execution_manifest.py" phase --manifest "$manifest" --phase "$phase"
run_formal_system OdinANN
python3 "$new/w1_cp05_r10_preservation.py" --preflight "$preflight" --phase after_OdinANN \
...
disk_unit=dv-w1-cp05-r10-diskann-stale
systemd-run --scope --collect --unit "$disk_unit" --uid ubuntu \
  ...
  env -i PATH=/usr/bin:/bin LANG=C LC_ALL=C PYTHONDONTWRITEBYTECODE=1 \
    W1_CP05_R10_CUMULATIVE_AUTHORIZED=1 W1_GLOBAL_LOCK_HELD=1 ATLAS_ROOT="$root" \
  ...
  prlimit --core=0:0 -- numactl --physcpubind=0-23 --membind=0 \
    "$new/w1_run_diskann_cp05_stale_r10.sh" "$query" "$cp05gt" \
      "$result/DiskANN/stale-cp05-10" "$artifact" "$runtime" "$preflight"

phase=final_preservation_audit
python3 "$new/w1_cumulative_r10_execution_manifest.py" phase --manifest "$manifest" --phase "$phase"
python3 "$new/w1_cp05_r10_preservation.py" --preflight "$preflight" --phase final \
...
phase=final_report
python3 "$new/w1_cumulative_r10_execution_manifest.py" phase --manifest "$manifest" --phase "$phase"
python3 "$new/w1_cp05_r10_finalize.py" --root "$root" --result-root "$result" \
...
python3 "$new/w1_cumulative_r10_execution_manifest.py" complete --manifest "$manifest" \
  --summary "$result/trajectory_summary.json"
touch "$result/FORMAL_W1_CP05_CUMULATIVE_COMPLETE"
notify 'Dynamic Vamana W1 CP05 cumulative R10 complete' \
