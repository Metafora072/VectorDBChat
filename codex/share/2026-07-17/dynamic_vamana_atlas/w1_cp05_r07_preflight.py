#!/usr/bin/env python3
"""Fail-closed preflight for CP05 cumulative trajectory R07 recovery."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-r07-preflight-v1"
...
R07_REPLAY_RUN = "pilot3_w1_cp05_trajectory_replay_r07"
...
def active_w1_processes() -> list[dict[str, Any]]:
    ...
    rows = []
    executable_tokens = (
        "run_w1_cp05_cumulative_trajectory_r07.sh",
        "start_w1_cp05_cumulative_r07_tmux.sh",
        "w1_run_cumulative_trajectory_r07.sh",
        "w1_cumulative_stage_worker_r07.sh",
        "w1_run_query_scope.sh",
        "w1_run_diskann_cp05_stale_r07.sh",
    )
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    r06_result = args.r06_terminal_result.resolve(strict=True)
    r06_replay_formal = args.r06_terminal_replay_formal.resolve(strict=True)
    ...
    result = args.r07_result_root.resolve(strict=False); formal = args.r07_formal_root.resolve(strict=False)
    replay_formal = args.r07_replay_formal_root.resolve(strict=False)
    ...
    require(r06_result == root / f"results/{R06_RUN}"
            and r06_replay_formal == root / "formal/pilot3_w1_cp05_trajectory_replay_r06",
            "R06 terminal capability path mismatch")
    require(result == root / f"results/{R07_RUN}" and formal == root / f"formal/{R07_RUN}"
            and replay_formal == root / f"formal/{R07_REPLAY_RUN}", "R07 capability path mismatch")
    require(output == result / "preflight/execution_preflight.json", "R07 preflight output path mismatch")
    require(delta == root / "datasets/sift10m/w1_trajectory/execution_deltas_r03",
            "R07 must reuse the accepted R03 delta root")
    require(replay_inputs == r03_result / "replay/inputs",
            "R07 must reuse the accepted R03 replay-input root")
    ...
    r06_execution_path = r06_result / "execution_manifest.json"; r06_execution = load(r06_execution_path)
    require((r06_execution.get("schema"), r06_execution.get("status"),
             r06_execution.get("stopped_phase"), r06_execution.get("exit_code"))
            == ("dynamic-vamana-w1-cp05-cumulative-r06-execution-v1",
                "stopped_failed", "replay_DGAI", 1), "R06 terminal identity mismatch")
    require(r06_execution.get("attempts", {}).get("DGAI_replay") == "sequential-cp80-06",
            "R06 replay attempt identity mismatch")
    r06_preservation_path = r06_result / "preflight/preservation_after_stop.json"
    r06_preservation = load(r06_preservation_path)
    require(r06_preservation.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r06-preservation-v1"
            and r06_preservation.get("status") == "pass" and not r06_preservation.get("mismatches"),
            "R06 stop-time preservation is not PASS")
    r06_query_gate_path = r06_result / "replay/DGAI/sequential-cp80-06/queries/cp00/query_gate.json"
    r06_query_gate = load(r06_query_gate_path)
    require(r06_query_gate.get("schema") == "dynamic-vamana-w1-query-identity-v2"
            and r06_query_gate.get("status") == "pass" and r06_query_gate.get("checkpoint") == "cp00",
            "R06 DGAI CP00 query gate is not PASS")
    r06_cp01 = r06_result / "replay/DGAI/sequential-cp80-06/stages/cp01"
    r06_forbidden = (list(r06_result.glob("**/STAGE_WORKER_OK"))
                     + list(r06_result.glob("**/markers.jsonl"))
                     + list(r06_result.glob("**/stage_evidence.json"))
                     + list(r06_result.glob("**/*checkpoint_evidence.json"))
                     + list(r06_result.glob("**/CUMULATIVE_TRAJECTORY_OK"))
                     + list(r06_result.glob("**/legacy_canary.json")))
    require(not r06_forbidden and not r06_cp01.exists()
            and not (r06_result / "replay/OdinANN").exists()
            and not (r06_result / "DGAI").exists() and not (r06_result / "OdinANN").exists()
            and not (r06_result / "DiskANN").exists()
            and not (root / f"formal/{R06_RUN}").exists(),
            "R06 advanced beyond the accepted CP00/pre-update boundary")
    require((r06_replay_formal / "DGAI/sequential-cp80-06/clone_manifest.json").is_file(),
            "R06 terminal clone evidence absent")
    ...
    require(canary_tests.get("schema") == "dynamic-vamana-w1-r04-minimal-input-canary-tests-v1"
            ...
            "R07 minimal input-canary tests not PASS")
    ...
    require(helper_record.get("realpath") == str(canary_helper)
            ...
            "R07 input-canary helper identity mismatch")

    primer_tests = load(primer_tests_path)
    require(primer_tests.get("schema") == "dynamic-vamana-w1-r07-stage-io-primer-tests-v1"
            ...
            "R07 same-scope stage-I/O primer test is not PASS")
    ...
    require(primer_helper.get("realpath") == str(stage_primer)
            ...
            "R07 stage-I/O primer helper identity mismatch")
    ...
    # R07 is fresh: before activation it may contain only recovery-gate evidence.
    if result.exists():
        require({entry.name for entry in result.iterdir()} <= {"preflight"},
                "R07 result contains attempt output")
        require(not any(result.glob("DGAI/**")) and not any(result.glob("OdinANN/**"))
                and not any(result.glob("DiskANN/**")), "R07 result attempt not fresh")
    require(not formal.exists() and not replay_formal.exists(), "R07 formal/replay clone target not fresh")
    require(not args.report.exists(), "R07 final report target is not fresh")
    ...
    report = {"schema": SCHEMA, "status": "pass", "run": R07_RUN,
        "attempts": {"DGAI": "trajectory-cp05-07", "OdinANN": "trajectory-cp05-07",
                     "DiskANN": "stale-cp05-07", "replay": "sequential-cp80-07"},
        "terminal_attempt": {"R03": {"execution_manifest": identity(r03_execution_path),
            ...
            "R06": {"execution_manifest": identity(r06_execution_path),
                "status": "stopped_failed", "stopped_phase": "replay_DGAI", "exit_code": 1,
                "stopped_before_input_canary": True, "update_api_called": False,
                "no_cp01_cp05_odin_formal_diskann": True,
                "cp00_query_gate": identity(r06_query_gate_path),
                "preservation_after_stop": identity(r06_preservation_path)}},
...
def self_test(args: argparse.Namespace) -> None:
    ...
    write_new(args.output, {"schema": "dynamic-vamana-w1-cp05-r07-preflight-self-test-v1",
...
def parser() -> argparse.ArgumentParser:
    ...
    for name in ("root", "artifact-manifest", "r03-result", "r03-formal",
                 ...
                 "r06-terminal-result", "r06-terminal-replay-formal",
                 "replay-base-root", "static-smoke", "static-smoke-revalidation",
                 "query-scope-tests", "input-canary-tests", "stage-io-primer-tests", "r07-result-root",
                 "r07-formal-root", "r07-replay-formal-root", "report", "output"):
