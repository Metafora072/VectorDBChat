#!/usr/bin/env bash
# R12-only accepted P1R07 DiskANN CP00 index queried against exact CP10 GT.
set -euo pipefail
export PYTHONDONTWRITEBYTECODE=1
[[ ${W1_CP10_DISKANN_R12_AUTHORIZED:-0} == 1 && ${W1_GLOBAL_LOCK_HELD:-0} == 1 ]] || {
  echo 'CP10 DiskANN R12 authorization/global lock absent' >&2; exit 64;
}
[[ $# == 7 ]] || {
  echo "usage: $0 QUERY CP10_GT RESULT_DIR ARTIFACT RUNTIME R10_PREFLIGHT R12_PREFLIGHT" >&2; exit 2;
}
query=$(realpath "$1"); gt=$(realpath "$2"); out=$(realpath -m "$3")
artifact=$(realpath "$4"); runtime=$(realpath "$5")
r10_preflight=$(realpath "$6"); r12_preflight=$(realpath "$7")
...
expected_out="$root/results/pilot3_sift10m_w1_cp10_trajectory_r12/DiskANN/stale-cp10-12"
expected_query="$root/datasets/sift10m/query.bin"
expected_gt="$root/groundtruth/sift10m/w1_trajectory/cp10/gt_cp10"
expected_r10_preflight="$root/results/pilot3_sift10m_w1_cp05_trajectory_r10/preflight/execution_preflight.json"
expected_r12_preflight="$root/results/pilot3_sift10m_w1_cp10_trajectory_r12/preflight/execution_preflight.json"
[[ $out == "$expected_out" && $query == "$expected_query" && $gt == "$expected_gt" \
  && $r10_preflight == "$expected_r10_preflight" && $r12_preflight == "$expected_r12_preflight" \
  && ! -e $out ]] || { echo 'DiskANN R12 target/input capability mismatch' >&2; exit 1; }
...
python3 "$new/w1_diskann_cp10_validate_r12.py" --result-dir "$out" --binary "$binary" \
  ...
  --execution-preflight "$r10_preflight" --closure-preflight "$r12_preflight" \
...
touch "$out/DISKANN_STALE_CP10_OK"
