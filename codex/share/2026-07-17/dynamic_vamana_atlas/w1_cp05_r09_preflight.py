#!/usr/bin/env python3
"""Fail-closed preflight for CP05 cumulative trajectory R09 recovery."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-r09-preflight-v1"
...
R09_REPLAY_RUN = "pilot3_w1_cp05_trajectory_replay_r09"
...
def verify_base(base_root: Path, system: str) -> dict[str, Any]:
    ...
    # of live source content/mode, all R09 lineage identities and base policy.
...
def active_w1_processes() -> list[dict[str, Any]]:
    ...
    rows = []
    executable_tokens = (
        "run_w1_cp05_cumulative_trajectory_r09.sh",
        "start_w1_cp05_cumulative_r09_tmux.sh",
        "w1_run_cumulative_trajectory_r09.sh",
        "w1_cumulative_stage_worker_r09.sh",
        "w1_run_query_scope.sh",
        "w1_run_diskann_cp05_stale_r09.sh",
    )
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result = args.r09_result_root.resolve(strict=False); formal = args.r09_formal_root.resolve(strict=False)
    replay_formal = args.r09_replay_formal_root.resolve(strict=False)
    ...
    require(result == root / f"results/{R09_RUN}" and formal == root / f"formal/{R09_RUN}"
            and replay_formal == root / f"formal/{R09_REPLAY_RUN}", "R09 capability path mismatch")
    require(output == result / "preflight/execution_preflight.json", "R09 preflight output path mismatch")
    require(delta == root / "datasets/sift10m/w1_trajectory/execution_deltas_r03",
            "R09 must reuse the accepted R03 delta root")
    require(replay_inputs == r03_result / "replay/inputs",
            "R09 must reuse the accepted R03 replay-input root")
    ...
    require(canary_tests.get("schema") == "dynamic-vamana-w1-canonical-input-canary-tests-v1"
            ...
            "R09 minimal input-canary tests not PASS")
    ...
    require(helper_record.get("realpath") == str(canary_helper)
            ...
            "R09 input-canary helper identity mismatch")

    primer_tests = load(primer_tests_path)
    require(primer_tests.get("schema") == "dynamic-vamana-w1-r09-stage-io-primer-tests-v1"
            ...
            "R09 same-scope stage-I/O primer test is not PASS")
    ...
    require(primer_helper.get("realpath") == str(stage_primer)
            ...
            "R09 stage-I/O primer helper identity mismatch")
    ...
    # R09 is fresh: before activation it may contain only recovery-gate evidence.
    if result.exists():
        require({entry.name for entry in result.iterdir()} <= {"preflight"},
                "R09 result contains attempt output")
        require(not any(result.glob("DGAI/**")) and not any(result.glob("OdinANN/**"))
                and not any(result.glob("DiskANN/**")), "R09 result attempt not fresh")
    require(not formal.exists() and not replay_formal.exists(), "R09 formal/replay clone target not fresh")
    require(not args.report.exists(), "R09 final report target is not fresh")
    ...
    report = {"schema": SCHEMA, "status": "pass", "run": R09_RUN,
        "attempts": {"DGAI": "trajectory-cp05-09", "OdinANN": "trajectory-cp05-09",
                     "DiskANN": "stale-cp05-09", "replay": "sequential-cp80-09"},
...
def self_test(args: argparse.Namespace) -> None:
    ...
    write_new(args.output, {"schema": "dynamic-vamana-w1-cp05-r09-preflight-self-test-v1",
...
def parser() -> argparse.ArgumentParser:
    ...
    for name in ("root", "artifact-manifest", "r03-result", "r03-formal",
                 ...
                 "query-scope-tests", "input-canary-tests", "stage-io-primer-tests", "r09-result-root",
                 "r09-formal-root", "r09-replay-formal-root", "report", "output"):
