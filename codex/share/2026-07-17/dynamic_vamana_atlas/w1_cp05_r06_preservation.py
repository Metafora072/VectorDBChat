#!/usr/bin/env python3
"""Revalidate every preflight-bound R06 input and infrastructure artifact."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-r06-preservation-v1"
...
def load_helper() -> Any:
    path = Path(__file__).with_name("w1_cp05_r06_preflight.py")
    spec = importlib.util.spec_from_file_location("w1_cp05_r06_preflight_import", path)
    require(spec is not None and spec.loader is not None, "cannot import R06 preflight helper")
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    require(preflight.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r06-preflight-v1"
            and preflight.get("status") == "pass", "R06 preflight not PASS")
    ...
    terminal_r05 = preflight["terminal_attempt"]["R05"]
    check_identity("terminal_R05_execution_manifest", terminal_r05["execution_manifest"])
    check_identity("terminal_R05_preservation_after_stop", terminal_r05["preservation_after_stop"])
    check_identity("terminal_R05_cp00_query_gate", terminal_r05["cp00_query_gate"])
    check_identity("terminal_R05_stage_local_canary", terminal_r05["stage_local_canary"])
    check_identity("terminal_R05_worker_identity", terminal_r05["worker_identity"])
    ...
    if mismatches: raise ValueError("R06 protected input/infrastructure changed")
    return report
