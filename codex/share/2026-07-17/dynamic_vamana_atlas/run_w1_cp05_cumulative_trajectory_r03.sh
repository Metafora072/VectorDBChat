#!/usr/bin/env bash
# Fail-closed R03 CP00->CP01->CP05 cumulative replay and formal execution.
set -euo pipefail
export PYTHONDONTWRITEBYTECODE=1
ulimit -c 0
[[ ${W1_CP05_R03_CUMULATIVE_AUTHORIZED:-0} == 1 ]] || { echo 'CP05 cumulative R03 authorization absent' >&2; exit 64; }
(( EUID == 0 )) || { echo 'CP05 cumulative R03 launcher must run as root' >&2; exit 1; }
...
run=pilot3_sift10m_w1_cp05_trajectory_r03
result="$root/results/$run"; formal="$root/formal/$run"
replay_run=pilot3_w1_cp05_trajectory_replay_r03; replay_formal="$root/formal/$replay_run"
...
tmp="$root/tmp/$run"
report="$new/../dynamic_vamana_w1_cp05_cumulative_trajectory_r03_results_0717.md"
...
cp01gt="$root/groundtruth/sift10m/w1_r03/gt_cp01"
...
stop_transient_units() {
  ...
  done < <(systemctl list-units --all --plain --no-legend 'dv-w1-cum-r03-*' 'dv-w1-cp05-r03-*' 2>/dev/null | awk '{print $1}')
}
...
on_exit() {
  ...
  if (( rc != 0 )); then
    ...
    if (( attempt_armed == 1 )) && [[ -f $manifest ]]; then
      python3 "$new/w1_cumulative_r03_execution_manifest.py" stop --manifest "$manifest" \
        --phase "$phase" --exit-code "$rc" || true
      if [[ -f $preflight && ! -e $result/preflight/preservation_after_stop.json ]]; then
        python3 "$new/w1_cp05_r03_preservation.py" --preflight "$preflight" \
          --phase "$phase" --output "$result/preflight/preservation_after_stop.json" || true
      fi
    fi
    notify "Dynamic Vamana CP05 cumulative R03 $phase failed" \
      "exit=$rc phase=$phase; fail-closed, no retry/continuation and CP10/CP20 remain HOLD"
  fi
}
trap on_exit EXIT

[[ ! -e $tmp ]] || { echo "R03 temporary target is not fresh: $tmp" >&2; exit 1; }
...
if systemctl list-units --all --plain --no-legend 'dv-w1-cum-r03-*' 'dv-w1-cp05-r03-*' 2>/dev/null | awk 'NF{bad=1} END{exit !bad}'; then
  echo 'stale CP05 cumulative R03 transient unit exists' >&2; exit 1
fi
for fresh_target in "$result" "$formal" "$replay_formal" "$delta" "$report"; do
  [[ ! -e $fresh_target ]] || { echo "R03 launch target is not fresh: $fresh_target" >&2; exit 1; }
done

compare_trees_exact_and_disjoint() {
  local accepted=$1 fresh=$2 label=$3 accepted_manifest=$4 fresh_manifest=$5
  python3 "$r01/w1_file_manifest.py" --root "$accepted" --output "$accepted_manifest"
  python3 "$r01/w1_file_manifest.py" --root "$fresh" --output "$fresh_manifest"
...
python3 "$new/w1_cp05_r03_preflight.py" self-test --scratch "$tmp" \
  --output "$tmp/r03_preflight_self_test.json"
...
phase=derive_and_freeze_r03_execution_deltas
python3 "$r02/w1_cumulative_prepare.py" --mode formal --dataset "$dataset" \
  --trajectory "$trajectory" --cp01 "$cp01" --output "$delta"
compare_trees_exact_and_disjoint "$r02_delta" "$delta" execution_deltas \
  "$tmp/execution_deltas_r02.tsv" "$tmp/execution_deltas_r03.tsv"
python3 "$r02/w1_mode_manifest.py" write --root "$delta" --output "$tmp/execution_deltas_mode_r03.tsv"

phase=derive_and_freeze_r03_replay_inputs
python3 "$r02/w1_cumulative_prepare.py" --mode replay --dataset "$root/datasets/sift1m" --output "$replay_inputs"
compare_trees_exact_and_disjoint "$r02_replay_inputs" "$replay_inputs" replay_inputs \
  "$tmp/replay_inputs_r02.tsv" "$tmp/replay_inputs_r03.tsv"
python3 "$r02/w1_mode_manifest.py" write --root "$replay_inputs" --output "$tmp/replay_inputs_mode_r03.tsv"

phase=query_scope_integration_fixtures
fixture_root="$tmp/query-scope-fixtures"
install -d -m 0770 -o ubuntu -g ubuntu "$fixture_root/DGAI" "$fixture_root/OdinANN" "$fixture_root/negative"
...
run_query_scope_fixture() {
  local system=$1 l_value=$2 io=$3 lower base_dir stem binary driver
  lower=${system,,}; base_dir="$replay_base_root/$system/cp00"
  stem="$fixture_root/$system/cp00_L${l_value}_r1"
  binary="$canonical/$system/search_disk_index"; driver="$canonical/$system/w1_canary"
  python3 "$r01/w1_file_manifest.py" --root "$base_dir/index" \
    --output "$fixture_root/$system/base_content_before.tsv"
  python3 "$r02/w1_mode_manifest.py" write --root "$base_dir/index" \
    --output "$fixture_root/$system/base_mode_before.tsv"
  W1_CP05_R03_CUMULATIVE_AUTHORIZED=1 "$new/w1_run_query_scope.sh" \
    --unit "dv-w1-cp05-r03-fixture-${lower}-l${l_value}" --system "$system" \
    --index-root "$base_dir/index" --query-binary "$binary" \
    --query "$replay_inputs/query_36.bin" --gt "$replay_inputs/gt_cp00_36" \
    --active-tags "$root/datasets/sift1m/active_cp00.tags.bin" --stem "$stem" \
    --l-value "$l_value" --memory-max 8G --resource-probe "$r01/resource_probe.py" \
    --query-worker "$r01/w1_query_worker.sh" --device "${ATLAS_NVME_MAJMIN:-259:10}"
  runuser -u ubuntu -- env PYTHONDONTWRITEBYTECODE=1 python3 "$new/w1_cumulative_evidence_r03.py" query-gate \
    --mode replay --system "$system" --checkpoint cp00 --result-dir "$fixture_root/$system" --prefix cp00 \
    --binary "$binary" --driver "$driver" --artifact-manifest "$artifact" \
    --index-content-manifest "$base_dir/base_content.tsv" --query "$replay_inputs/query_36.bin" \
    --gt "$replay_inputs/gt_cp00_36" --active-tags "$root/datasets/sift1m/active_cp00.tags.bin" \
    --ls "$l_value" --repeats 1 --threads 1 --io-engine "$io" \
    --device "${ATLAS_NVME_MAJMIN:-259:10}" --expected-nq 36 --expected-k 10 \
    --expected-active-count "$active_n" --output "$fixture_root/$system/query_gate.json"
  python3 "$r01/w1_file_manifest.py" --root "$base_dir/index" \
    --output "$fixture_root/$system/base_content_after.tsv"
  python3 "$r02/w1_mode_manifest.py" write --root "$base_dir/index" \
    --output "$fixture_root/$system/base_mode_after.tsv"
}
run_query_scope_fixture DGAI 64 aio
run_query_scope_fixture OdinANN 29 uring

negative_resource="$fixture_root/negative/no_primer.resources.json"
systemd-run --scope --collect --unit dv-w1-cp05-r03-fixture-negative --uid ubuntu \
  ...
  prlimit --core=0:0 -- numactl --physcpubind=0-23 --membind=0 \
    python3 "$r01/resource_probe.py" --output "$negative_resource" --interval-ms 25 \
      --space-root "$replay_base_root/DGAI/cp00/index" -- /bin/sleep 0.20
python3 "$new/w1_query_scope_tests.py" \
  --dgai-gate "$fixture_root/DGAI/query_gate.json" --odin-gate "$fixture_root/OdinANN/query_gate.json" \
  --negative-resource "$negative_resource" \
  --dgai-content-before "$fixture_root/DGAI/base_content_before.tsv" \
  --dgai-content-after "$fixture_root/DGAI/base_content_after.tsv" \
  --dgai-mode-before "$fixture_root/DGAI/base_mode_before.tsv" \
  --dgai-mode-after "$fixture_root/DGAI/base_mode_after.tsv" \
  --odin-content-before "$fixture_root/OdinANN/base_content_before.tsv" \
  --odin-content-after "$fixture_root/OdinANN/base_content_after.tsv" \
  --odin-mode-before "$fixture_root/OdinANN/base_mode_before.tsv" \
  --odin-mode-after "$fixture_root/OdinANN/base_mode_after.tsv" \
  --device "${ATLAS_NVME_MAJMIN:-259:10}" --output "$tmp/query_scope_tests.json"
...
install -m 0444 "$tmp/query_scope_tests.json" "$result/preflight/query_scope_tests.json"
cp -a "$fixture_root" "$result/preflight/query_scope_fixtures"
chown -R root:root "$result/preflight/query_scope_fixtures"
find "$result/preflight/query_scope_fixtures" -type f -exec chmod 0444 {} +
find "$result/preflight/query_scope_fixtures" -type d -exec chmod 0555 {} +

phase=r03_execution_preflight
python3 "$new/w1_cp05_r03_preflight.py" validate --root "$root" --artifact-manifest "$artifact" \
  --r01-result "$r01_result" --r01-formal "$r01_formal" \
  --r02-result "$r02_result" --r02-formal "$r02_formal" --r02-replay-formal "$r02_replay_formal" \
  --r02-delta-root "$r02_delta" --r02-replay-input-root "$r02_replay_inputs" \
  ...
  --r03-result-root "$result" --r03-formal-root "$formal" \
  --r03-replay-formal-root "$replay_formal" --r03-replay-input-root "$replay_inputs" \
  --r03-delta-root "$delta" --report "$report" --device "${ATLAS_NVME_MAJMIN:-259:10}" --output "$preflight"
python3 "$new/w1_cumulative_r03_execution_manifest.py" activate --manifest "$manifest" \
  --preflight "$preflight" \
  ...
  --static-smoke "$smoke_output" --old-attempt-manifest "$r02_manifest" --phase evidence_self_test
attempt_armed=1
install -m 0444 "$tmp/r03_preflight_self_test.json" "$result/preflight/r03_preflight_self_test.json"
install -m 0444 "$tmp/execution_deltas_r02.tsv" "$result/preflight/execution_deltas_r02.tsv"
...
install -m 0444 "$tmp/replay_inputs_r02.tsv" "$result/preflight/replay_inputs_r02.tsv"
...
runner="$new/w1_run_cumulative_trajectory_r03.sh"
...
run_replay_system() {
  ...
  W1_CP05_R03_CUMULATIVE_AUTHORIZED=1 "$runner" --mode replay --system "$system" \
    --run-name "$replay_run" --attempt sequential-cp80-03 --base-index "$base" \
...
run_formal_system() {
  ...
  W1_CP05_R03_CUMULATIVE_AUTHORIZED=1 "$runner" --mode formal --system "$system" \
    --run-name "$run" --attempt trajectory-cp05-03 --base-index "$base" \
...
phase=replay_DGAI
python3 "$new/w1_cumulative_r03_execution_manifest.py" phase --manifest "$manifest" --phase "$phase"
...
python3 "$new/w1_cp05_r03_preservation.py" --preflight "$preflight" --phase after_DGAI \
  --output "$result/preflight/preservation_after_DGAI.json"

phase=formal_OdinANN_cumulative
python3 "$new/w1_cumulative_r03_execution_manifest.py" phase --manifest "$manifest" --phase "$phase"
run_formal_system OdinANN
python3 "$new/w1_cp05_r03_preservation.py" --preflight "$preflight" --phase after_OdinANN \
...
disk_unit=dv-w1-cp05-r03-diskann-stale
systemd-run --scope --collect --unit "$disk_unit" --uid ubuntu \
  ...
  env -i PATH=/usr/bin:/bin LANG=C LC_ALL=C PYTHONDONTWRITEBYTECODE=1 \
    W1_CP05_R03_CUMULATIVE_AUTHORIZED=1 W1_GLOBAL_LOCK_HELD=1 ATLAS_ROOT="$root" \
    ATLAS_NVME_MAJMIN="${ATLAS_NVME_MAJMIN:-259:10}" W1_EXPECTED_SCOPE="$disk_unit.scope" \
  prlimit --core=0:0 -- numactl --physcpubind=0-23 --membind=0 \
    "$new/w1_run_diskann_cp05_stale_r03.sh" "$query" "$cp05gt" \
      "$result/DiskANN/stale-cp05-03" "$artifact" "$runtime" "$preflight"

phase=final_preservation_audit
python3 "$new/w1_cumulative_r03_execution_manifest.py" phase --manifest "$manifest" --phase "$phase"
python3 "$new/w1_cp05_r03_preservation.py" --preflight "$preflight" --phase final \
...
phase=final_report
python3 "$new/w1_cumulative_r03_execution_manifest.py" phase --manifest "$manifest" --phase "$phase"
python3 "$new/w1_cp05_r03_finalize.py" --root "$root" --result-root "$result" \
...
python3 "$new/w1_cumulative_r03_execution_manifest.py" complete --manifest "$manifest" \
  --summary "$result/trajectory_summary.json"
touch "$result/FORMAL_W1_CP05_CUMULATIVE_COMPLETE"
notify 'Dynamic Vamana W1 CP05 cumulative R03 complete' \
