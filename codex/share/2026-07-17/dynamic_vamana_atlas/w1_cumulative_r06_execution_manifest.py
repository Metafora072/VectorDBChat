#!/usr/bin/env python3
"""Create or transition the fail-closed CP05 cumulative R06 execution manifest."""
...
def main() -> None:
    ...
    if args.action == "activate":
        ...
        if args.manifest.exists() or any(path is None or not path.is_file() for path in required):
            raise SystemExit("R06 activation requires fresh manifest and all frozen gate evidence")
        preflight = json.loads(args.preflight.read_text())
        if preflight.get("status") != "pass":
            raise SystemExit("preflight did not pass")
        old = json.loads(args.old_attempt_manifest.read_text())
        if (old.get("schema") != "dynamic-vamana-w1-cp05-cumulative-r05-execution-v1"
        ...
        payload: dict[str, object] = {
            "schema": "dynamic-vamana-w1-cp05-cumulative-r06-execution-v1",
            "status": "running",
            ...
            "formal_root": str(atlas_root / "formal/pilot3_sift10m_w1_cp05_trajectory_r06"),
            "replay_run": "pilot3_w1_cp05_trajectory_replay_r06",
            "replay_formal_root": str(atlas_root / "formal/pilot3_w1_cp05_trajectory_replay_r06"),
            ...
            "terminal_r05_manifest_realpath": str(args.old_attempt_manifest.resolve()),
            "terminal_r05_manifest_sha256": sha256(args.old_attempt_manifest),
            "authorized_sequence": ["CP00", "CP01", "CP05"],
            "attempts": {
                "DGAI_replay": "sequential-cp80-06",
                "OdinANN_replay": "sequential-cp80-06",
                "DGAI_formal": "trajectory-cp05-06",
                "OdinANN_formal": "trajectory-cp05-06",
                "DiskANN": "stale-cp05-06",
            },
            "old_attempt_reused": False,
            "r05_clone_result_attempt_reused": False,
