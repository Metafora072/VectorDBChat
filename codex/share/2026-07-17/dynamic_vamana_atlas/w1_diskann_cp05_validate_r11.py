#!/usr/bin/env python3
"""R11 identity-v2 validation for DiskANN CP00-index versus CP05-GT stale control."""
...
def main() -> None:
    ...
    if (closure_preflight.get("schema") != "dynamic-vamana-w1-cp05-diskann-closure-r11-preflight-v1"
            or closure_preflight.get("status") != "pass"):
        raise SystemExit("DiskANN R11 closure preflight is not accepted")
    bound_r10_preflight = closure_preflight.get("r10", {}).get("execution_preflight")
    if (not isinstance(bound_r10_preflight, dict)
        ...
        raise SystemExit("DiskANN R11/R10 preflight anti-tamper anchor mismatch")
    ...
    report = {
        "schema": "dynamic-vamana-w1-diskann-cp05-stale-r11-v1",
        "status": "pass",
        ...
        "identities": {
            ...
            "r11_closure_preflight": standard_identity(args.closure_preflight),
        },
        "points": points,
