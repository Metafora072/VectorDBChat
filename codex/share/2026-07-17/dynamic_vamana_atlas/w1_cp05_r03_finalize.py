#!/usr/bin/env python3
"""Tamper-evident finalizer for CP05 cumulative trajectory R03 outputs."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-trajectory-r03-summary-v1"
RUN = "pilot3_sift10m_w1_cp05_trajectory_r03"
REPLAY_RUN = "pilot3_w1_cp05_trajectory_replay_r03"
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    old = module("w1_cumulative_finalize_r03_import", "w1_cumulative_finalize.py")
    prehelper = module("w1_cp05_r03_preflight_finalize_import", "w1_cp05_r03_preflight.py")
    ...
    require(result == root / f"results/{RUN}" and formal_root == root / f"formal/{RUN}",
            "R03 finalizer capability path mismatch")
    require(replay_result in (result / "replay", root / f"results/{REPLAY_RUN}"),
            "R03 replay result root mismatch")
    expected_report = Path(__file__).parent.parent / "dynamic_vamana_w1_cp05_cumulative_trajectory_r03_results_0717.md"
    require(args.output_report.resolve(strict=False) == expected_report.resolve(strict=False),
            "R03 report output path mismatch")
    ...
    require(preflight.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r03-preflight-v1"
            and preflight.get("status") == "pass", "R03 preflight invalid")
    preservation_path = args.preservation.resolve(strict=True); preservation = load(preservation_path)
    require(preservation.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r03-preservation-v1"
            and preservation.get("status") == "pass" and not preservation.get("mismatches"),
            "R03 preservation invalid")
    ...
    for system in ("DGAI", "OdinANN"):
        replay_attempt = replay_result / system / "sequential-cp80-03"
        replay_work = replay_work_root / system / "sequential-cp80-03"
        ...
        attempt = result / system / "trajectory-cp05-03"; work = formal_root / system / "trajectory-cp05-03"
    ...
    disk_path = result / "DiskANN/stale-cp05-03/stale_control.json"; disk = old.require_pass(disk_path)
    ...
    summary = {"schema": SCHEMA, "status": "pass",
        "decision_boundary": "CP05 cumulative R03 only; CP10/CP20 remain HOLD",
    ...
    lines = ["# Dynamic Vamana W1 CP05 累计 Trajectory R03 实验结果", "", "## 裁决", "",
