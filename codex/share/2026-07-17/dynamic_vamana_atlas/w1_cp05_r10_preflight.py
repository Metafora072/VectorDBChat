#!/usr/bin/env python3
"""Fail-closed preflight for CP05 cumulative trajectory R10 recovery."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-r10-preflight-v1"
R03_RUN = "pilot3_sift10m_w1_cp05_trajectory_r03"
R07_RUN = "pilot3_sift10m_w1_cp05_trajectory_r07"
R09_RUN = "pilot3_sift10m_w1_cp05_trajectory_r09"
R10_RUN = "pilot3_sift10m_w1_cp05_trajectory_r10"
R10_REPLAY_RUN = "pilot3_w1_cp05_trajectory_replay_r10"
...
def verify_base(base_root: Path, system: str) -> dict[str, Any]:
    ...
    # of live source content/mode, all R10 lineage identities and base policy.
...
def active_w1_processes() -> list[dict[str, Any]]:
    ...
    rows = []
    executable_tokens = (
        "run_w1_cp05_cumulative_trajectory_r10.sh",
        "start_w1_cp05_cumulative_r10_tmux.sh",
        "w1_run_cumulative_trajectory_r10.sh",
        "w1_cumulative_stage_worker_r10.sh",
        "w1_run_query_scope.sh",
        "w1_run_diskann_cp05_stale_r10.sh",
    )
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    r07_result = args.r07_terminal_result.resolve(strict=True)
    r07_replay_formal = args.r07_terminal_replay_formal.resolve(strict=True)
    r09_result = args.r09_terminal_result.resolve(strict=True)
    r09_replay_formal = args.r09_terminal_replay_formal.resolve(strict=True)
    ...
    result = args.r10_result_root.resolve(strict=False); formal = args.r10_formal_root.resolve(strict=False)
    replay_formal = args.r10_replay_formal_root.resolve(strict=False)
    ...
    require(r07_result == root / f"results/{R07_RUN}"
            and r07_replay_formal == root / "formal/pilot3_w1_cp05_trajectory_replay_r07",
            "R07 terminal capability path mismatch")
    require(r09_result == root / f"results/{R09_RUN}"
            and r09_replay_formal == root / "formal/pilot3_w1_cp05_trajectory_replay_r09",
            "R09 terminal capability path mismatch")
    require(result == root / f"results/{R10_RUN}" and formal == root / f"formal/{R10_RUN}"
            and replay_formal == root / f"formal/{R10_REPLAY_RUN}", "R10 capability path mismatch")
    require(output == result / "preflight/execution_preflight.json", "R10 preflight output path mismatch")
    require(delta == root / "datasets/sift10m/w1_trajectory/execution_deltas_r03",
            "R10 must reuse the accepted R03 delta root")
    require(replay_inputs == r03_result / "replay/inputs",
            "R10 must reuse the accepted R03 replay-input root")
    ...
    r07_execution_path = r07_result / "execution_manifest.json"; r07_execution = load(r07_execution_path)
    require((r07_execution.get("schema"), r07_execution.get("status"),
             r07_execution.get("stopped_phase"), r07_execution.get("exit_code"))
            == ("dynamic-vamana-w1-cp05-cumulative-r07-execution-v1",
                "stopped_failed", "replay_DGAI", 1), "R07 terminal identity mismatch")
    require(r07_execution.get("attempts", {}).get("DGAI_replay") == "sequential-cp80-07",
            "R07 replay attempt identity mismatch")
    r07_preservation_path = r07_result / "preflight/preservation_after_stop.json"
    r07_preservation = load(r07_preservation_path)
    require(r07_preservation.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r07-preservation-v1"
            and r07_preservation.get("status") == "pass" and not r07_preservation.get("mismatches"),
            "R07 stop-time preservation is not PASS")
    r07_query_gate_path = r07_result / "replay/DGAI/sequential-cp80-07/queries/cp00/query_gate.json"
    r07_query_gate = load(r07_query_gate_path)
    require(r07_query_gate.get("schema") == "dynamic-vamana-w1-query-identity-v2"
            and r07_query_gate.get("status") == "pass" and r07_query_gate.get("checkpoint") == "cp00",
            "R07 DGAI CP00 query gate is not PASS")
    r07_attempt = r07_result / "replay/DGAI/sequential-cp80-07"
    r07_cp01 = r07_attempt / "stages/cp01"
    r07_canary_path = r07_cp01 / "input_canary/canary.json"
    r07_worker_path = r07_cp01 / "worker_identity.json"
    r07_legacy_path = r07_cp01 / "legacy_canary.json"
    r07_primer_path = r07_attempt / "cp01_stage_io_primer.json"
    r07_resources_path = r07_attempt / "cp01_stage_resources.json"
    r07_canary = load(r07_canary_path); r07_worker = load(r07_worker_path)
    r07_legacy = load(r07_legacy_path); r07_primer = load(r07_primer_path)
    require(r07_canary.get("schema") == "dynamic-vamana-w1-r04-input-canary-v1"
            and r07_canary.get("status") == "pass" and r07_canary.get("allowed_readable") is True
            and r07_canary.get("denied")
            and all(row.get("open_refused") is True for row in r07_canary["denied"]),
            "R07 terminal input-canary content is not PASS")
    require(r07_worker.get("schema") == "dynamic-vamana-w1-cumulative-stage-worker-identity-v1"
            and r07_worker.get("status") == "pass"
            and r07_worker.get("incremental_replacements") == 16
            and r07_worker.get("primitive_mutations") == 32,
            "R07 completed update worker identity is invalid")
    require(r07_legacy.get("schema") == "dynamic-vamana-w1-canary-collection-v3"
            and r07_legacy.get("logical_replacements") == 16,
            "R07 strict legacy collector output is invalid")
    require(r07_primer.get("schema") == "dynamic-vamana-w1-stage-io-primer-v1"
            and r07_primer.get("status") == "pass" and r07_primer.get("bytes_read") == 4096,
            "R07 stage-I/O primer evidence is invalid")
    require((r07_cp01 / "STAGE_WORKER_OK").is_file()
            and (r07_cp01 / "markers.jsonl").is_file()
            and (r07_cp01 / "active_audit.json").is_file()
            and (r07_cp01 / "fresh_probe.json").is_file()
            and r07_resources_path.is_file(),
            "R07 completed update evidence is incomplete")
    r07_forbidden = (list(r07_result.glob("**/stage_evidence.json"))
                     + list(r07_result.glob("**/*checkpoint_evidence.json"))
                     + list(r07_result.glob("**/CUMULATIVE_TRAJECTORY_OK"))
                     + list(r07_result.glob("**/cp01_checkpoint_evidence.json")))
    require(not r07_forbidden and not (r07_attempt / "queries/cp01").exists()
            and not (r07_result / "replay/OdinANN").exists()
            and not (r07_result / "DGAI").exists() and not (r07_result / "OdinANN").exists()
            and not (r07_result / "DiskANN").exists()
            and not (root / f"formal/{R07_RUN}").exists(),
            "R07 advanced beyond the accepted post-update/pre-stage-evidence boundary")
    require((r07_replay_formal / "DGAI/sequential-cp80-07/clone_manifest.json").is_file(),
            "R07 terminal clone evidence absent")

    r09_execution_path = r09_result / "execution_manifest.json"; r09_execution = load(r09_execution_path)
    require((r09_execution.get("schema"), r09_execution.get("status"),
             r09_execution.get("stopped_phase"), r09_execution.get("exit_code"))
            == ("dynamic-vamana-w1-cp05-cumulative-r09-execution-v1",
                "stopped_failed", "formal_DGAI_cumulative", 1), "R09 terminal identity mismatch")
    require(r09_execution.get("attempts") == {
                "DGAI_replay": "sequential-cp80-09", "OdinANN_replay": "sequential-cp80-09",
                "DGAI_formal": "trajectory-cp05-09", "OdinANN_formal": "trajectory-cp05-09",
                "DiskANN": "stale-cp05-09"}, "R09 attempt identities mismatch")
    r09_preservation_path = r09_result / "preflight/preservation_after_stop.json"
    r09_preservation = load(r09_preservation_path)
    require(r09_preservation.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r09-preservation-v1"
            and r09_preservation.get("status") == "pass"
            and r09_preservation.get("phase") == "formal_DGAI_cumulative"
            and not r09_preservation.get("mismatches"), "R09 stop-time preservation is not PASS")
    r09_stage_evidence: dict[str, dict[str, Any]] = {}
    for system in ("DGAI", "OdinANN"):
        attempt = r09_result / f"replay/{system}/sequential-cp80-09"
        require((attempt / "CUMULATIVE_TRAJECTORY_OK").is_file(), f"R09 {system} replay is incomplete")
        require((r09_replay_formal / f"{system}/sequential-cp80-09/clone_manifest.json").is_file(),
                f"R09 {system} replay clone evidence absent")
        for checkpoint in ("cp01", "cp05"):
            ...
            require(evidence.get("schema") == "dynamic-vamana-w1-cumulative-stage-evidence-v1"
                    ...
                    and (evidence_path.parent / "STAGE_WORKER_OK").is_file(),
                    f"R09 {system} {checkpoint} replay evidence is invalid")
            r09_stage_evidence[f"{system}_{checkpoint}"] = identity(evidence_path)
    require(not (r09_result / "DGAI").exists() and not (r09_result / "OdinANN").exists()
            and not (r09_result / "DiskANN").exists()
            and not (root / f"formal/{R09_RUN}").exists(),
            "R09 advanced beyond completed replay into formal output")
    ...
    require(canary_tests.get("schema") == "dynamic-vamana-w1-canonical-input-canary-tests-v1"
            ...
            "R10 minimal input-canary tests not PASS")
    ...
    require(helper_record.get("realpath") == str(canary_helper)
            ...
            "R10 input-canary helper identity mismatch")

    primer_tests = load(primer_tests_path)
    require(primer_tests.get("schema") == "dynamic-vamana-w1-r10-stage-io-primer-tests-v1"
            ...
            "R10 same-scope stage-I/O primer test is not PASS")
    ...
    require(primer_helper.get("realpath") == str(stage_primer)
            ...
            "R10 stage-I/O primer helper identity mismatch")
    ...
    # R10 is fresh: before activation it may contain only recovery-gate evidence.
    if result.exists():
        require({entry.name for entry in result.iterdir()} <= {"preflight"},
                "R10 result contains attempt output")
        require(not any(result.glob("DGAI/**")) and not any(result.glob("OdinANN/**"))
                and not any(result.glob("DiskANN/**")), "R10 result attempt not fresh")
    require(not formal.exists() and not replay_formal.exists(), "R10 formal/replay clone target not fresh")
    require(not args.report.exists(), "R10 final report target is not fresh")
    ...
    report = {"schema": SCHEMA, "status": "pass", "run": R10_RUN,
        "attempts": {"DGAI": "trajectory-cp05-10", "OdinANN": "trajectory-cp05-10",
                     "DiskANN": "stale-cp05-10", "replay": "sequential-cp80-10"},
        "terminal_attempt": {"R03": {"execution_manifest": identity(r03_execution_path),
            ...
            "R07": {"execution_manifest": identity(r07_execution_path),
                "status": "stopped_failed", "stopped_phase": "replay_DGAI", "exit_code": 1,
                "update_api_completed": True, "stage_io_accounting_closed": True,
                "stage_evidence_rejected_for_schema": True,
                "no_cp01_checkpoint_cp05_odin_formal_diskann": True,
                "cp00_query_gate": identity(r07_query_gate_path),
                "stage_local_canary": identity(r07_canary_path),
                "worker_identity": identity(r07_worker_path),
                "legacy_canary": identity(r07_legacy_path),
                "stage_io_primer": identity(r07_primer_path),
                "stage_resources": identity(r07_resources_path),
                "preservation_after_stop": identity(r07_preservation_path)},
            "R09": {"execution_manifest": identity(r09_execution_path),
                "status": "stopped_failed", "stopped_phase": "formal_DGAI_cumulative", "exit_code": 1,
                "both_replays_completed": True, "formal_update_not_started": True,
                "stage_evidence": r09_stage_evidence,
                "preservation_after_stop": identity(r09_preservation_path)}},
...
def self_test(args: argparse.Namespace) -> None:
    ...
    write_new(args.output, {"schema": "dynamic-vamana-w1-cp05-r10-preflight-self-test-v1",
...
def parser() -> argparse.ArgumentParser:
    ...
    for name in ("root", "artifact-manifest", "r03-result", "r03-formal",
                 ...
                 "r07-terminal-result", "r07-terminal-replay-formal",
                 "r09-terminal-result", "r09-terminal-replay-formal",
                 "replay-base-root", "static-smoke", "static-smoke-revalidation",
                 "query-scope-tests", "input-canary-tests", "stage-io-primer-tests", "r10-result-root",
                 "r10-formal-root", "r10-replay-formal-root", "report", "output"):
