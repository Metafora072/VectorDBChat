#!/usr/bin/env python3
"""Revalidate every preflight-bound R09 input and infrastructure artifact."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-r09-preservation-v1"
...
def load_helper() -> Any:
    path = Path(__file__).with_name("w1_cp05_r09_preflight.py")
    spec = importlib.util.spec_from_file_location("w1_cp05_r09_preflight_import", path)
    require(spec is not None and spec.loader is not None, "cannot import R09 preflight helper")
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    require(preflight.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r09-preflight-v1"
            and preflight.get("status") == "pass", "R09 preflight not PASS")
    ...
    if mismatches: raise ValueError("R09 protected input/infrastructure changed")
    return report
