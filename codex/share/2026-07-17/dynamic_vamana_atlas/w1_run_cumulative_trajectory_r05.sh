#!/usr/bin/env bash
# R05-only shared CP00 -> CP01 -> CP05 state machine for DGAI and OdinANN.
set -euo pipefail
...
[[ ${W1_CP05_R05_CUMULATIVE_AUTHORIZED:-0} == 1 && ${W1_GLOBAL_LOCK_HELD:-0} == 1 ]] || {
  echo 'R05 cumulative trajectory/global-lock capability absent' >&2; exit 64;
}
...
if [[ ${arg[mode]} == formal ]]; then
  ...
  [[ ${arg[run-name]} == pilot3_sift10m_w1_cp05_trajectory_r05 && ${arg[attempt]} == trajectory-cp05-05 ]] || {
    echo 'R05 formal run/attempt identity mismatch' >&2; exit 1;
  }
else
  [[ ${arg[cp01-count]} == 16 && ${arg[cp05-count]} == 64 ]] || {
    echo 'sequential replay counts must be 16 then 64 delta' >&2; exit 1;
  }
  [[ ${arg[run-name]} == pilot3_w1_cp05_trajectory_replay_r05 && ${arg[attempt]} == sequential-cp80-05 ]] || {
    echo 'R05 replay run/attempt identity mismatch' >&2; exit 1;
  }
fi
...
[[ -f $evidence && -x $new/w1_cumulative_stage_worker_r05.sh \
...
if [[ ${arg[mode]} == formal ]]; then
  result=$(realpath -m "$root/results/pilot3_sift10m_w1_cp05_trajectory_r05/${arg[system]}/${arg[attempt]}")
else
  result=$(realpath -m "$root/results/pilot3_sift10m_w1_cp05_trajectory_r05/replay/${arg[system]}/${arg[attempt]}")
fi
expected_work="$root/formal/${arg[run-name]}/${arg[system]}/${arg[attempt]}"
if [[ ${arg[mode]} == formal ]]; then expected_result="$root/results/pilot3_sift10m_w1_cp05_trajectory_r05/${arg[system]}/${arg[attempt]}"
else expected_result="$root/results/pilot3_sift10m_w1_cp05_trajectory_r05/replay/${arg[system]}/${arg[attempt]}"; fi
...
[[ $base == "$expected_base" ]] || { echo "R05 base identity mismatch: $base" >&2; exit 1; }
...
if [[ ${arg[mode]} == replay ]]; then
  clone_helper="$new/w1_clone_replay_base_r05.sh"
else
...
query_checkpoint() {
  ...
  for l in "${query_ls[@]}"; do
    for repetition in 1 2 3; do
      ...
      local unit="dv-w1-cum-r03-r05-${arg[mode]}-${system_lower}-${checkpoint}-l${l}-r${repetition}"
...
inaccessible_canary() {
  ...
  local unit="dv-w1-cum-r05-${arg[mode]}-${system_lower}-${checkpoint}-input-canary"
...
run_stage() {
  ...
  local stage_result="$result/stages/$checkpoint" resource="$result/${checkpoint}_stage_resources.json"
  local unit="dv-w1-cum-r05-${arg[mode]}-${system_lower}-${checkpoint}-update"
  ...
  systemd-run --wait --collect --pipe --unit "$unit" --uid ubuntu --property=Type=exec \
    ...
    numactl --physcpubind=0-23 --membind=0 \
      ...
      "$new/w1_cumulative_stage_worker_r05.sh" --mode "${arg[mode]}" --system "${arg[system]}" --stage "$checkpoint" \
