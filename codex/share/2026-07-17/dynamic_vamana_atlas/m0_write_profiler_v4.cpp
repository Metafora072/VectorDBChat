...
extern "C" ssize_t sendfile(int out_fd,int in_fd,off_t*in_offset,size_t count){using F=ssize_t(*)(int,int,off_t*,size_t);static F f=next_symbol<F>("sendfile");off_t out_offset=lseek(out_fd,0,SEEK_CUR);ssize_t r=f(out_fd,in_fd,in_offset,count);if(r>0)record("posix","sendfile",out_fd,out_offset>=0?uint64_t(out_offset):0,r);return r;}
