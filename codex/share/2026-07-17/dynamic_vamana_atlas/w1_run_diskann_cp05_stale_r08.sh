#!/usr/bin/env bash
# R08-only frozen DiskANN CP00 index queried against exact CP05 GT.
set -euo pipefail
export PYTHONDONTWRITEBYTECODE=1
[[ ${W1_CP05_R08_CUMULATIVE_AUTHORIZED:-0} == 1 && ${W1_GLOBAL_LOCK_HELD:-0} == 1 ]] || {
  echo 'CP05 cumulative R08 authorization/global lock absent' >&2; exit 64;
}
...
old=$(cd "$new/../../2026-08-15/dynamic_vamana_atlas" && pwd)
r02=${ATLAS_W1_R02_CHAT:-/home/ubuntu/pz/VectorDB/chat/codex/share/2026-08-16/dynamic_vamana_atlas}
expected_out="$root/results/pilot3_sift10m_w1_cp05_trajectory_r08/DiskANN/stale-cp05-08"
expected_gt="$root/groundtruth/sift10m/w1_trajectory/cp05/gt_cp05"
expected_query="$root/datasets/sift10m/query.bin"
expected_preflight="$root/results/pilot3_sift10m_w1_cp05_trajectory_r08/preflight/execution_preflight.json"
...
base_dir="$root/formal/pilot3_sift10m_p1r08/f0/DiskANN/p1r08-01/index"
...
python3 "$new/w1_diskann_cp05_validate_r08.py" --result-dir "$out" --binary "$binary" \
