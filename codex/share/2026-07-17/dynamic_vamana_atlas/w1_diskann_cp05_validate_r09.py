#!/usr/bin/env python3
"""R09 identity-v2 validation for DiskANN CP00-index versus CP05-GT stale control."""
...
def validate_accepted_lineage(args: argparse.Namespace, artifact: dict[str, Any]) -> dict[str, object]:
    ...
    if preflight.get("status") != "pass" or preflight.get("schema") != "dynamic-vamana-w1-cp05-cumulative-r09-preflight-v1":
        raise ValueError("CP05 R09 execution preflight is not accepted")
    ...
    accepted_runtime = require_standard_identity(
        lineage["runtime_manifest"], args.runtime_manifest, "R09 runtime manifest"
    )
    accepted_loader = require_standard_identity(lineage["loader_tests"], None, "R09 loader tests")
    accepted_environment = require_standard_identity(
        lineage["runtime_environment"], None, "R09 runtime environment"
    )
    accepted_base_content = require_standard_identity(
        lineage["base_content_manifest"], None, "R09 base content manifest"
    )
    accepted_base_mode = require_standard_identity(
        lineage["base_mode_manifest"], None, "R09 base mode manifest"
    )
    ...
    if (runtime.get("schema") != "dynamic-vamana-w1-diskann-runtime-manifest-v1"
        ...
        raise ValueError("R09 runtime manifest verdict/schema invalid")
...
def main() -> None:
    ...
    report = {
        "schema": "dynamic-vamana-w1-diskann-cp05-stale-r09-v1",
        "status": "pass",
