...
if [[ -e $result ]]; then
  [[ -d $result && -d $result/input_canary && ! -L $result/input_canary \
    ...
    echo 'stage result lacks the exact R09 input-canary evidence' >&2; exit 1;
  }
  ...
  [[ ${existing[*]} == 'input_canary input_canary/canary.json input_canary/canary.log' ]] || {
    echo 'stage result contains state beyond the R09 input canary' >&2; exit 1;
  }
else
...
python3 - "$trace" "$delta_manifest" "${arg[expected-count]}" "${arg[delta-start]}" "$full" "$before_tags" "$after_tags" "$probes" "$probe_spec" "$local_spec" "$global_spec" "$result/worker_identity.json" "$scope" "${arg[mode]}" "${arg[system]}" "${arg[stage]}" "$attempt" "$new/w1_cumulative_stage_worker_r09.sh" "${arg[forbidden-paths]}" <<'PY'
