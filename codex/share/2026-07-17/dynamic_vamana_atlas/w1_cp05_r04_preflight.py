#!/usr/bin/env python3
"""Fail-closed preflight for CP05 cumulative trajectory R04 recovery."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-r04-preflight-v1"
R03_RUN = "pilot3_sift10m_w1_cp05_trajectory_r03"
R04_RUN = "pilot3_sift10m_w1_cp05_trajectory_r04"
R04_REPLAY_RUN = "pilot3_w1_cp05_trajectory_replay_r04"
...
def active_w1_processes() -> list[dict[str, Any]]:
    ...
    rows = []
    executable_tokens = (
        "run_w1_cp05_cumulative_trajectory_r04.sh",
        "start_w1_cp05_cumulative_r04_tmux.sh",
        "w1_run_cumulative_trajectory_r04.sh",
        "w1_cumulative_stage_worker.sh",
        "w1_run_query_scope.sh",
        "w1_run_diskann_cp05_stale_r04.sh",
    )
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result = args.r04_result_root.resolve(strict=False); formal = args.r04_formal_root.resolve(strict=False)
    replay_formal = args.r04_replay_formal_root.resolve(strict=False)
    ...
    require(result == root / f"results/{R04_RUN}" and formal == root / f"formal/{R04_RUN}"
            and replay_formal == root / f"formal/{R04_REPLAY_RUN}", "R04 capability path mismatch")
    require(output == result / "preflight/execution_preflight.json", "R04 preflight output path mismatch")
    require(delta == root / "datasets/sift10m/w1_trajectory/execution_deltas_r03",
            "R04 must reuse the accepted R03 delta root")
    require(replay_inputs == r03_result / "replay/inputs",
            "R04 must reuse the accepted R03 replay-input root")
    ...
    require(canary_tests.get("schema") == "dynamic-vamana-w1-r04-minimal-input-canary-tests-v1"
            ...
            "R04 minimal input-canary tests not PASS")
    ...
    require(helper_record.get("realpath") == str(canary_helper)
            ...
            "R04 input-canary helper identity mismatch")
    ...
    # R04 is fresh: before activation it may contain only recovery-gate evidence.
    if result.exists():
        require({entry.name for entry in result.iterdir()} <= {"preflight"},
                "R04 result contains attempt output")
        require(not any(result.glob("DGAI/**")) and not any(result.glob("OdinANN/**"))
                and not any(result.glob("DiskANN/**")), "R04 result attempt not fresh")
    require(not formal.exists() and not replay_formal.exists(), "R04 formal/replay clone target not fresh")
    require(not args.report.exists(), "R04 final report target is not fresh")
    ...
    report = {"schema": SCHEMA, "status": "pass", "run": R04_RUN,
        "attempts": {"DGAI": "trajectory-cp05-04", "OdinANN": "trajectory-cp05-04",
                     "DiskANN": "stale-cp05-04", "replay": "sequential-cp80-04"},
...
def self_test(args: argparse.Namespace) -> None:
    ...
    write_new(args.output, {"schema": "dynamic-vamana-w1-cp05-r04-preflight-self-test-v1",
...
def parser() -> argparse.ArgumentParser:
    ...
    for name in ("root", "artifact-manifest", "r03-result", "r03-formal",
                 ...
                 "query-scope-tests", "input-canary-tests", "r04-result-root",
                 "r04-formal-root", "r04-replay-formal-root", "report", "output"):
