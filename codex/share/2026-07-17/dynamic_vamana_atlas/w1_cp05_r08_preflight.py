#!/usr/bin/env python3
"""Fail-closed preflight for CP05 cumulative trajectory R08 recovery."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-r08-preflight-v1"
R03_RUN = "pilot3_sift10m_w1_cp05_trajectory_r03"
R07_RUN = "pilot3_sift10m_w1_cp05_trajectory_r07"
R08_RUN = "pilot3_sift10m_w1_cp05_trajectory_r08"
R08_REPLAY_RUN = "pilot3_w1_cp05_trajectory_replay_r08"
STATIC_SMOKE_SHA256 = "5c9f2189a5c37c29d052c3593bc5cdd4f635b050cb6bbbf60a857b49b7be09c3"
R02_TOOL_ROOT = Path(
    "/home/ubuntu/pz/VectorDB/chat/codex/share/2026-08-16/dynamic_vamana_atlas"
)
...
def verify_base(base_root: Path, system: str) -> dict[str, Any]:
    ...
    # of live source content/mode, all R08 lineage identities and base policy.
...
def active_w1_processes() -> list[dict[str, Any]]:
    ...
    rows = []
    executable_tokens = (
        "run_w1_cp05_cumulative_trajectory_r08.sh",
        "start_w1_cp05_cumulative_r08_tmux.sh",
        "w1_run_cumulative_trajectory_r08.sh",
        "w1_cumulative_stage_worker_r08.sh",
        "w1_run_query_scope.sh",
        "w1_run_diskann_cp05_stale_r08.sh",
    )
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result = args.r08_result_root.resolve(strict=False); formal = args.r08_formal_root.resolve(strict=False)
    replay_formal = args.r08_replay_formal_root.resolve(strict=False)
    ...
    require(result == root / f"results/{R08_RUN}" and formal == root / f"formal/{R08_RUN}"
            and replay_formal == root / f"formal/{R08_REPLAY_RUN}", "R08 capability path mismatch")
    require(output == result / "preflight/execution_preflight.json", "R08 preflight output path mismatch")
    require(delta == root / "datasets/sift10m/w1_trajectory/execution_deltas_r03",
            "R08 must reuse the accepted R03 delta root")
    require(replay_inputs == r03_result / "replay/inputs",
            "R08 must reuse the accepted R03 replay-input root")
    ...
    require(canary_tests.get("schema") == "dynamic-vamana-w1-canonical-input-canary-tests-v1"
            ...
            "R08 minimal input-canary tests not PASS")
    ...
    require(helper_record.get("realpath") == str(canary_helper)
            ...
            "R08 input-canary helper identity mismatch")

    primer_tests = load(primer_tests_path)
    require(primer_tests.get("schema") == "dynamic-vamana-w1-r08-stage-io-primer-tests-v1"
            ...
            "R08 same-scope stage-I/O primer test is not PASS")
    ...
    require(primer_helper.get("realpath") == str(stage_primer)
            ...
            "R08 stage-I/O primer helper identity mismatch")
    ...
    # R08 is fresh: before activation it may contain only recovery-gate evidence.
    if result.exists():
        require({entry.name for entry in result.iterdir()} <= {"preflight"},
                "R08 result contains attempt output")
        require(not any(result.glob("DGAI/**")) and not any(result.glob("OdinANN/**"))
                and not any(result.glob("DiskANN/**")), "R08 result attempt not fresh")
    require(not formal.exists() and not replay_formal.exists(), "R08 formal/replay clone target not fresh")
    require(not args.report.exists(), "R08 final report target is not fresh")
    ...
    report = {"schema": SCHEMA, "status": "pass", "run": R08_RUN,
        "attempts": {"DGAI": "trajectory-cp05-08", "OdinANN": "trajectory-cp05-08",
                     "DiskANN": "stale-cp05-08", "replay": "sequential-cp80-08"},
...
def self_test(args: argparse.Namespace) -> None:
    ...
    write_new(args.output, {"schema": "dynamic-vamana-w1-cp05-r08-preflight-self-test-v1",
...
def parser() -> argparse.ArgumentParser:
    ...
    for name in ("root", "artifact-manifest", "r03-result", "r03-formal",
                 ...
                 "query-scope-tests", "input-canary-tests", "stage-io-primer-tests", "r08-result-root",
                 "r08-formal-root", "r08-replay-formal-root", "report", "output"):
