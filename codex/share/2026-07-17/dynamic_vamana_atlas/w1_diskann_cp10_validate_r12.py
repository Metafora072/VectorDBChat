#!/usr/bin/env python3
"""R12 identity-v2 validation for DiskANN CP00-index versus CP10-GT stale control."""
...
def validate_accepted_lineage(args: argparse.Namespace, artifact: dict[str, Any]) -> dict[str, object]:
    ...
    # CP10 GT did not exist in R10's CP05 execution scope; R12 binds it below.
    return {
...
def main() -> None:
    ...
    if (closure_preflight.get("schema") != "dynamic-vamana-w1-cp10-r12-preflight-v1"
            or closure_preflight.get("status") != "pass"):
        raise SystemExit("DiskANN R12 execution preflight is not accepted")
    bound_r10_preflight = closure_preflight.get("r10_execution_preflight")
    if (not isinstance(bound_r10_preflight, dict)
        ...
        raise SystemExit("DiskANN R12/R10 preflight anti-tamper anchor mismatch")
    bound_cp10_gt = closure_preflight.get("cp10_gt")
    if (not isinstance(bound_cp10_gt, dict)
            or bound_cp10_gt.get("realpath") != str(args.gt.resolve())
            or bound_cp10_gt.get("sha256") != sha256(args.gt)):
        raise SystemExit("DiskANN R12 CP10 GT anchor mismatch")
    ...
    report = {
        "schema": "dynamic-vamana-w1-diskann-cp10-stale-r12-v1",
        "status": "pass",
        "classification": "stale-static negative control",
        "rank_with_update_throughput": False,
        "stale_result_ids_may_be_inactive_at_cp10": True,
        ...
        "identities": {
            ...
            "cp10_gt": {"realpath": str(args.gt.resolve()), "sha256": sha256(args.gt)},
            ...
            "r12_closure_preflight": standard_identity(args.closure_preflight),
        },
        "points": points,
