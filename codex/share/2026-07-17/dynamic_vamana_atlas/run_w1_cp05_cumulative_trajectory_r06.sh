#!/usr/bin/env bash
# Fail-closed R06 CP00->CP01->CP05 cumulative replay and formal execution.
set -euo pipefail
export PYTHONDONTWRITEBYTECODE=1
ulimit -c 0
[[ ${W1_CP05_R06_CUMULATIVE_AUTHORIZED:-0} == 1 ]] || { echo 'CP05 cumulative R06 authorization absent' >&2; exit 64; }
(( EUID == 0 )) || { echo 'CP05 cumulative R06 launcher must run as root' >&2; exit 1; }
...
run=pilot3_sift10m_w1_cp05_trajectory_r06
result="$root/results/$run"; formal="$root/formal/$run"
replay_run=pilot3_w1_cp05_trajectory_replay_r06; replay_formal="$root/formal/$replay_run"
...
r05_result="$root/results/$r05_run"
r05_replay_formal="$root/formal/pilot3_w1_cp05_trajectory_replay_r05"
r05_manifest="$r05_result/execution_manifest.json"
...
tmp="$root/tmp/$run"
report="$new/../dynamic_vamana_w1_cp05_cumulative_trajectory_r06_results_0717.md"
...
stop_transient_units() {
  ...
  done < <(systemctl list-units --all --plain --no-legend 'dv-w1-cum-r06-*' 'dv-w1-cp05-r06-*' 2>/dev/null | awk '{print $1}')
}
...
on_exit() {
  ...
  if (( rc != 0 )); then
    ...
    if (( attempt_armed == 1 )) && [[ -f $manifest ]]; then
      python3 "$new/w1_cumulative_r06_execution_manifest.py" stop --manifest "$manifest" \
        --phase "$phase" --exit-code "$rc" || true
      if [[ -f $preflight && ! -e $result/preflight/preservation_after_stop.json ]]; then
        python3 "$new/w1_cp05_r06_preservation.py" --preflight "$preflight" \
          --phase "$phase" --output "$result/preflight/preservation_after_stop.json" || true
      fi
    fi
    notify "Dynamic Vamana CP05 cumulative R06 $phase failed" \
      "exit=$rc phase=$phase; fail-closed, no retry/continuation and CP10/CP20 remain HOLD"
  fi
}
trap on_exit EXIT

[[ ! -e $tmp ]] || { echo "R06 temporary target is not fresh: $tmp" >&2; exit 1; }
...
if systemctl list-units --all --plain --no-legend 'dv-w1-cum-r06-*' 'dv-w1-cp05-r06-*' 2>/dev/null | awk 'NF{bad=1} END{exit !bad}'; then
  echo 'stale CP05 cumulative R06 transient unit exists' >&2; exit 1
fi
for fresh_target in "$result" "$formal" "$replay_formal" "$report"; do
  [[ ! -e $fresh_target ]] || { echo "R06 launch target is not fresh: $fresh_target" >&2; exit 1; }
done

phase=immutable_replay_base_recovery
python3 "$new/w1_cp05_r06_preflight.py" self-test --scratch "$tmp" \
  --output "$tmp/r06_preflight_self_test.json"
...
positive_unit=dv-w1-cp05-r06-canary-positive
...
primer_unit=dv-w1-cp05-r06-stage-primer-positive
...
payload={"schema":"dynamic-vamana-w1-r06-stage-io-primer-tests-v1","status":"pass",
...
phase=r06_execution_preflight
python3 "$new/w1_cp05_r06_preflight.py" validate --root "$root" --artifact-manifest "$artifact" \
  ...
  --r05-terminal-result "$r05_result" --r05-terminal-replay-formal "$r05_replay_formal" \
  ...
  --r06-result-root "$result" --r06-formal-root "$formal" \
  --r06-replay-formal-root "$replay_formal" --report "$report" \
  --device "${ATLAS_NVME_MAJMIN:-259:10}" --output "$preflight"
python3 "$new/w1_cumulative_r06_execution_manifest.py" activate --manifest "$manifest" \
  --preflight "$preflight" \
  ...
  --static-smoke "$smoke_output" --old-attempt-manifest "$r05_manifest" --phase evidence_self_test
attempt_armed=1
install -m 0444 "$tmp/r06_preflight_self_test.json" "$result/preflight/r06_preflight_self_test.json"
...
runner="$new/w1_run_cumulative_trajectory_r06.sh"
...
run_replay_system() {
  ...
  W1_CP05_R06_CUMULATIVE_AUTHORIZED=1 "$runner" --mode replay --system "$system" \
    --run-name "$replay_run" --attempt sequential-cp80-06 --base-index "$base" \
...
run_formal_system() {
  ...
  W1_CP05_R06_CUMULATIVE_AUTHORIZED=1 "$runner" --mode formal --system "$system" \
    --run-name "$run" --attempt trajectory-cp05-06 --base-index "$base" \
...
phase=replay_DGAI
python3 "$new/w1_cumulative_r06_execution_manifest.py" phase --manifest "$manifest" --phase "$phase"
...
python3 "$new/w1_cp05_r06_preservation.py" --preflight "$preflight" --phase after_DGAI \
  --output "$result/preflight/preservation_after_DGAI.json"

phase=formal_OdinANN_cumulative
python3 "$new/w1_cumulative_r06_execution_manifest.py" phase --manifest "$manifest" --phase "$phase"
run_formal_system OdinANN
python3 "$new/w1_cp05_r06_preservation.py" --preflight "$preflight" --phase after_OdinANN \
...
disk_unit=dv-w1-cp05-r06-diskann-stale
systemd-run --scope --collect --unit "$disk_unit" --uid ubuntu \
  ...
  env -i PATH=/usr/bin:/bin LANG=C LC_ALL=C PYTHONDONTWRITEBYTECODE=1 \
    W1_CP05_R06_CUMULATIVE_AUTHORIZED=1 W1_GLOBAL_LOCK_HELD=1 ATLAS_ROOT="$root" \
  ...
  prlimit --core=0:0 -- numactl --physcpubind=0-23 --membind=0 \
    "$new/w1_run_diskann_cp05_stale_r06.sh" "$query" "$cp05gt" \
      "$result/DiskANN/stale-cp05-06" "$artifact" "$runtime" "$preflight"

phase=final_preservation_audit
python3 "$new/w1_cumulative_r06_execution_manifest.py" phase --manifest "$manifest" --phase "$phase"
python3 "$new/w1_cp05_r06_preservation.py" --preflight "$preflight" --phase final \
...
phase=final_report
python3 "$new/w1_cumulative_r06_execution_manifest.py" phase --manifest "$manifest" --phase "$phase"
python3 "$new/w1_cp05_r06_finalize.py" --root "$root" --result-root "$result" \
...
python3 "$new/w1_cumulative_r06_execution_manifest.py" complete --manifest "$manifest" \
  --summary "$result/trajectory_summary.json"
touch "$result/FORMAL_W1_CP05_CUMULATIVE_COMPLETE"
notify 'Dynamic Vamana W1 CP05 cumulative R06 complete' \
