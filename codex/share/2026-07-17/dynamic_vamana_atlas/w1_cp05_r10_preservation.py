#!/usr/bin/env python3
"""Revalidate every preflight-bound R10 input and infrastructure artifact."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-r10-preservation-v1"
...
def load_helper() -> Any:
    path = Path(__file__).with_name("w1_cp05_r10_preflight.py")
    spec = importlib.util.spec_from_file_location("w1_cp05_r10_preflight_import", path)
    require(spec is not None and spec.loader is not None, "cannot import R10 preflight helper")
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    require(preflight.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r10-preflight-v1"
            and preflight.get("status") == "pass", "R10 preflight not PASS")
    ...
    terminal_r07 = preflight["terminal_attempt"]["R07"]
    check_identity("terminal_R07_execution_manifest", terminal_r07["execution_manifest"])
    check_identity("terminal_R07_preservation_after_stop", terminal_r07["preservation_after_stop"])
    check_identity("terminal_R07_cp00_query_gate", terminal_r07["cp00_query_gate"])
    check_identity("terminal_R07_stage_local_canary", terminal_r07["stage_local_canary"])
    check_identity("terminal_R07_worker_identity", terminal_r07["worker_identity"])
    check_identity("terminal_R07_legacy_canary", terminal_r07["legacy_canary"])
    check_identity("terminal_R07_stage_io_primer", terminal_r07["stage_io_primer"])
    check_identity("terminal_R07_stage_resources", terminal_r07["stage_resources"])
    terminal_r09 = preflight["terminal_attempt"]["R09"]
    check_identity("terminal_R09_execution_manifest", terminal_r09["execution_manifest"])
    check_identity("terminal_R09_preservation_after_stop", terminal_r09["preservation_after_stop"])
    for name, evidence in terminal_r09["stage_evidence"].items():
        check_identity(f"terminal_R09_stage_evidence_{name}", evidence)
    ...
    check_identity("stage_io_primer_tests", preflight["stage_io_primer_tests"])
    ...
    primer_tests = load(Path(preflight["stage_io_primer_tests"]["realpath"]))
    primer_helper = primer_tests["helper"]; primer_helper_path = Path(primer_helper["realpath"])
    if (primer_helper_path.stat().st_size != primer_helper["size_bytes"]
            or sha256(primer_helper_path) != primer_helper["sha256"]):
        mismatches.append({"name": "stage_io_primer_helper", "reason": "stage I/O primer helper changed"})
    ...
    if mismatches: raise ValueError("R10 protected input/infrastructure changed")
    return report
