#!/usr/bin/env python3
"""Create or transition the fail-closed CP05 cumulative R10 execution manifest."""
...
def main() -> None:
    ...
    if args.action == "activate":
        ...
        if args.manifest.exists() or any(path is None or not path.is_file() for path in required):
            raise SystemExit("R10 activation requires fresh manifest and all frozen gate evidence")
        preflight = json.loads(args.preflight.read_text())
        if preflight.get("status") != "pass":
            raise SystemExit("preflight did not pass")
        old = json.loads(args.old_attempt_manifest.read_text())
        if (old.get("schema") != "dynamic-vamana-w1-cp05-cumulative-r09-execution-v1"
                or old.get("status") != "stopped_failed"
                or old.get("stopped_phase") != "formal_DGAI_cumulative"):
            raise SystemExit("old cumulative attempt is not the accepted terminal R09 formal_DGAI stop")
        atlas_root = args.manifest.resolve().parents[2]
        payload: dict[str, object] = {
            "schema": "dynamic-vamana-w1-cp05-cumulative-r10-execution-v1",
            "status": "running",
            ...
            "formal_root": str(atlas_root / "formal/pilot3_sift10m_w1_cp05_trajectory_r10"),
            "replay_run": "pilot3_w1_cp05_trajectory_replay_r10",
            "replay_formal_root": str(atlas_root / "formal/pilot3_w1_cp05_trajectory_replay_r10"),
            ...
            "terminal_r09_manifest_realpath": str(args.old_attempt_manifest.resolve()),
            "terminal_r09_manifest_sha256": sha256(args.old_attempt_manifest),
            "authorized_sequence": ["CP00", "CP01", "CP05"],
            "attempts": {
                "DGAI_replay": "sequential-cp80-10",
                "OdinANN_replay": "sequential-cp80-10",
                "DGAI_formal": "trajectory-cp05-10",
                "OdinANN_formal": "trajectory-cp05-10",
                "DiskANN": "stale-cp05-10",
            },
            "old_attempt_reused": False,
            "r09_clone_result_attempt_reused": False,
            "r07_clone_result_attempt_reused": False,
