#!/usr/bin/env python3
"""Atomic terminal state for the final R13 CP20 trajectory."""
...
if args.command=="activate":
    if args.manifest.exists(): raise SystemExit("R13 manifest reuse refused")
    d={"schema":"dynamic-vamana-w1-cp20-r13-execution-v1","run":"pilot3_sift10m_w1_cp20_trajectory_r13",
       "status":"running","phase":"preflight_complete","preflight":{"realpath":str(args.preflight.resolve()),"sha256":sha(args.preflight)},
       "started_unix_ns":time.time_ns(),"post_cp20":"STOP_AND_AWAIT_REVIEW"}
else:
    d=json.loads(args.manifest.read_text())
    if d.get("status")!="running": raise SystemExit("R13 manifest is terminal")
