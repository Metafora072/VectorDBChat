#!/usr/bin/env python3
"""Revalidate every preflight-bound R07 input and infrastructure artifact."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-r07-preservation-v1"
...
def load_helper() -> Any:
    path = Path(__file__).with_name("w1_cp05_r07_preflight.py")
    spec = importlib.util.spec_from_file_location("w1_cp05_r07_preflight_import", path)
    require(spec is not None and spec.loader is not None, "cannot import R07 preflight helper")
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    require(preflight.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r07-preflight-v1"
            and preflight.get("status") == "pass", "R07 preflight not PASS")
    ...
    terminal_r06 = preflight["terminal_attempt"]["R06"]
    check_identity("terminal_R06_execution_manifest", terminal_r06["execution_manifest"])
    check_identity("terminal_R06_preservation_after_stop", terminal_r06["preservation_after_stop"])
    check_identity("terminal_R06_cp00_query_gate", terminal_r06["cp00_query_gate"])
    ...
    if mismatches: raise ValueError("R07 protected input/infrastructure changed")
    return report
