#!/usr/bin/env bash
# R11-only accepted P1R07 DiskANN CP00 index queried against exact CP05 GT.
set -euo pipefail
export PYTHONDONTWRITEBYTECODE=1
[[ ${W1_CP05_DISKANN_R11_AUTHORIZED:-0} == 1 && ${W1_GLOBAL_LOCK_HELD:-0} == 1 ]] || {
  echo 'CP05 DiskANN R11 authorization/global lock absent' >&2; exit 64;
}
[[ $# == 7 ]] || {
  echo "usage: $0 QUERY CP05_GT RESULT_DIR ARTIFACT RUNTIME R10_PREFLIGHT R11_PREFLIGHT" >&2; exit 2;
}
query=$(realpath "$1"); gt=$(realpath "$2"); out=$(realpath -m "$3")
artifact=$(realpath "$4"); runtime=$(realpath "$5")
r10_preflight=$(realpath "$6"); r11_preflight=$(realpath "$7")
...
expected_out="$root/results/pilot3_sift10m_w1_cp05_diskann_closure_r11/DiskANN/stale-cp05-11"
...
expected_r11_preflight="$root/results/pilot3_sift10m_w1_cp05_diskann_closure_r11/preflight/execution_preflight.json"
[[ $out == "$expected_out" && $query == "$expected_query" && $gt == "$expected_gt" \
  && $r10_preflight == "$expected_r10_preflight" && $r11_preflight == "$expected_r11_preflight" \
  && ! -e $out ]] || { echo 'DiskANN R11 target/input capability mismatch' >&2; exit 1; }
...
python3 "$new/w1_diskann_cp05_validate_r11.py" --result-dir "$out" --binary "$binary" \
  ...
  --execution-preflight "$r10_preflight" --closure-preflight "$r11_preflight" \
