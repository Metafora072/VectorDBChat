...
* ``checkpoint`` binds a checkpoint state, update-worker identity, active-tag
  audit, and query gates into a cumulative CP01/CP20 evidence record.
    ...
    for search_l in ls_values:
        for repeat in repeats:
            ...
            paths = {
                ...
                "io_primer": Path(f"{stem}.io_primer.json"),
            }
            ...
            primer = json_load(paths["io_primer"])
            ...
            primer_before = primer.get("prime_file", {})
            require(primer.get("schema") == "dynamic-vamana-w1-query-io-primer-v1"
                    and primer.get("status") == "pass" and primer.get("bytes_requested") == 4096
                    and primer.get("bytes_read") == 4096 and primer.get("return_code") == 0
                    and primer.get("direct_io") is True
                    and primer.get("resource_probe_started_after_primer") is True
                    and primer.get("primer_excluded_from_query_delta") is True,
                    "query I/O primer report is not PASS")
            require(primer.get("prime_file_after") == primer_before,
                    "query I/O primer changed target identity")
            require(primer.get("index_root_realpath") == str(space_root)
                    and primer_before.get("realpath") == str(space_root / "index_disk.index")
                    and primer_before.get("device_major_minor") == args.device
                    and primer_before.get("mode_octal") in ("0444", "0600")
                    and int(primer_before.get("link_count", 0)) == 1,
                    "query I/O primer target capability mismatch")
            samples = resource.get("samples", [])
            require(isinstance(samples, list) and len(samples) >= 2,
                    "query resource evidence needs bracketing samples")
            baseline_row = device_row(samples[0], args.device)
            final_row = device_row(samples[-1], args.device)
            baseline_rbytes, baseline_rios = int(baseline_row.get("rbytes", -1)), int(baseline_row.get("rios", -1))
            final_rbytes, final_rios = int(final_row.get("rbytes", -1)), int(final_row.get("rios", -1))
            require(baseline_rbytes >= 4096 and baseline_rios >= 1,
                    "query resource baseline does not include the 4 KiB primer")
            ...
            point = {
                ...
                "primer_report_sha256": sha256(paths["io_primer"]),
                "baseline_target_read_bytes": baseline_rbytes,
                "baseline_target_read_ios": baseline_rios,
                "final_target_read_bytes": final_rbytes,
                "final_target_read_ios": final_rios,
                "query_target_read_bytes_delta": rbytes,
                "query_target_read_ios_delta": rios,
...
def freeze(args: argparse.Namespace) -> dict[str, Any]:
    ...
    checkpoint_evidence, checkpoint_artifact = require_pass_json(
        args.checkpoint_evidence, "CP20 checkpoint evidence"
    )
    require(checkpoint_evidence.get("checkpoint") == "cp20", "freeze requires CP20 checkpoint evidence")
    ...
    args.content_before = output_dir / "cp20_freeze_content_before.tsv"
    args.mode_before = output_dir / "cp20_freeze_mode_before.tsv"
    args.content_after = output_dir / "cp20_freeze_content_after.tsv"
    args.mode_after = output_dir / "cp20_freeze_mode_after.tsv"
    args.marker = attempt / "IMMUTABLE_TRAJECTORY_CP20_OK"
    args.output = output_dir / "cp20_freeze_evidence.json"
    ...
    require(checkpoint_evidence["state_content_manifest"]["sha256"] == sha256(args.content_before),
            "index content changed between CP20 checkpoint and freeze")
    require(checkpoint_evidence["state_mode_manifest"]["sha256"] == sha256(args.mode_before),
            "index mode changed between CP20 checkpoint and freeze")
    ...
    marker_payload = b"IMMUTABLE_TRAJECTORY_CP20_OK\n"
    write_new_bytes(args.marker, marker_payload)
    report = {
        "schema": "dynamic-vamana-w1-cp20-freeze-v1",
        "status": "pass",
        "system": args.system,
        "mode": args.mode,
        "checkpoint": "cp20",
...
def stage_evidence(args: argparse.Namespace) -> dict[str, Any]:
    ...
    intervals = {
        "replay": {"cp01": (0, 16), "cp20": (800_000, 800_000)},
        "formal": {"cp01": (0, 80_000), "cp20": (800_000, 800_000)},
    }
...
def checkpoint(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # R13 starts from an externally frozen R12 CP10 clone.  The parent CP10
    # checkpoint is bound by the R13 preflight, not synthesized in this tree.
    ...
    report = {
        ...
        "same_clone_as_previous": False,
        "distinct_worker_from_previous": False,
    }
...
def self_test(args: argparse.Namespace) -> dict[str, Any]:
    ...
    (private / "index_disk.index").write_bytes(b"x" * 4096)
    os.chmod(private / "index_disk.index", 0o600)
    ...
    _, _, private_files = tree_objects(private)
    index_manifest.write_bytes(content_payload(content_rows(private, private_files)))
    ...
    Path(f"{stem}.resources.json").write_text(json.dumps({
        ...
        "samples": [
            ...
            {"cgroup_memory_events": {"oom": 0, "oom_kill": 0, "oom_group_kill": 0},
             "cgroup_io_stat": [{"device": "1:2", "rbytes": 8192, "rios": 2}]},
        ],
    }))
    synthetic_prime = {
        "realpath": str((private / "index_disk.index").resolve()), "device_major_minor": "1:2",
        "inode": (private / "index_disk.index").stat().st_ino, "size_bytes": 4096,
        "uid": os.geteuid(), "gid": os.getegid(), "mode_octal": "0600", "link_count": 1,
        "mtime_ns": (private / "index_disk.index").stat().st_mtime_ns,
    }
    Path(f"{stem}.io_primer.json").write_text(json.dumps({
        "schema": "dynamic-vamana-w1-query-io-primer-v1", "status": "pass",
        "accounting_role": "accounting infrastructure", "index_root_realpath": str(private.resolve()),
        "prime_file": synthetic_prime, "prime_file_after": synthetic_prime,
        "bytes_requested": 4096, "bytes_read": 4096, "return_code": 0, "direct_io": True,
        "resource_probe_started_after_primer": True, "primer_excluded_from_query_delta": True,
    }))
...
def parser() -> argparse.ArgumentParser:
    ...
    query.add_argument("--checkpoint", choices=("cp00", "cp01", "cp20"), required=True)
    ...
    freeze_parser = sub.add_parser("freeze", help="freeze CP20 clone and prove owner denials")
    ...
    stage_parser.add_argument("--checkpoint", choices=("cp01", "cp20"), required=True)
    ...
    checkpoint_parser = sub.add_parser("checkpoint", help="bind CP01/CP20 cumulative evidence")
    ...
    checkpoint_parser.add_argument("--checkpoint", choices=("cp01", "cp20"), required=True)
