...
* ``checkpoint`` binds a checkpoint state, update-worker identity, active-tag
  audit, and query gates into a cumulative CP01/CP10 evidence record.
...
def freeze(args: argparse.Namespace) -> dict[str, Any]:
    ...
    checkpoint_evidence, checkpoint_artifact = require_pass_json(
        args.checkpoint_evidence, "CP10 checkpoint evidence"
    )
    require(checkpoint_evidence.get("checkpoint") == "cp10", "freeze requires CP10 checkpoint evidence")
    ...
    args.content_before = output_dir / "cp10_freeze_content_before.tsv"
    args.mode_before = output_dir / "cp10_freeze_mode_before.tsv"
    args.content_after = output_dir / "cp10_freeze_content_after.tsv"
    args.mode_after = output_dir / "cp10_freeze_mode_after.tsv"
    args.marker = attempt / "IMMUTABLE_TRAJECTORY_CP10_OK"
    args.output = output_dir / "cp10_freeze_evidence.json"
    ...
    require(checkpoint_evidence["state_content_manifest"]["sha256"] == sha256(args.content_before),
            "index content changed between CP10 checkpoint and freeze")
    require(checkpoint_evidence["state_mode_manifest"]["sha256"] == sha256(args.mode_before),
            "index mode changed between CP10 checkpoint and freeze")
    ...
    marker_payload = b"IMMUTABLE_TRAJECTORY_CP10_OK\n"
    write_new_bytes(args.marker, marker_payload)
    report = {
        "schema": "dynamic-vamana-w1-cp10-freeze-v1",
        "status": "pass",
        "system": args.system,
        "mode": args.mode,
        "checkpoint": "cp10",
...
def stage_evidence(args: argparse.Namespace) -> dict[str, Any]:
    ...
    intervals = {
        "replay": {"cp01": (0, 16), "cp10": (400_000, 400_000)},
        "formal": {"cp01": (0, 80_000), "cp10": (400_000, 400_000)},
    }
...
def checkpoint(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # R12 starts from an externally frozen R10 CP05 clone.  The parent CP05
    # checkpoint is bound by the R12 preflight, not synthesized in this tree.
...
def parser() -> argparse.ArgumentParser:
    ...
    query.add_argument("--checkpoint", choices=("cp00", "cp01", "cp10"), required=True)
    ...
    freeze_parser = sub.add_parser("freeze", help="freeze CP10 clone and prove owner denials")
    ...
    stage_parser.add_argument("--checkpoint", choices=("cp01", "cp10"), required=True)
    ...
    checkpoint_parser = sub.add_parser("checkpoint", help="bind CP01/CP10 cumulative evidence")
    ...
    checkpoint_parser.add_argument("--checkpoint", choices=("cp01", "cp10"), required=True)
