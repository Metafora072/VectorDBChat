#!/usr/bin/env python3
"""R13 identity-v2 validation for DiskANN CP00-index versus CP20-GT stale control."""
...
def validate_accepted_lineage(args: argparse.Namespace, artifact: dict[str, Any]) -> dict[str, object]:
    ...
    if preflight.get("status") != "pass" or preflight.get("schema") != "dynamic-vamana-w1-cp05-cumulative-r10-preflight-v1":
        raise ValueError("CP05 R10 execution preflight is not accepted")
    ...
    accepted_runtime = require_standard_identity(
        lineage["runtime_manifest"], args.runtime_manifest, "R10 runtime manifest"
    )
    accepted_loader = require_standard_identity(lineage["loader_tests"], None, "R10 loader tests")
    accepted_environment = require_standard_identity(
        lineage["runtime_environment"], None, "R10 runtime environment"
    )
    accepted_base_content = require_standard_identity(
        lineage["base_content_manifest"], None, "R10 base content manifest"
    )
    accepted_base_mode = require_standard_identity(
        lineage["base_mode_manifest"], None, "R10 base mode manifest"
    )
    ...
    if (runtime.get("schema") != "dynamic-vamana-w1-diskann-runtime-manifest-v1"
        ...
        raise ValueError("R10 runtime manifest verdict/schema invalid")
    ...
    # CP20 GT did not exist in R10's CP05 execution scope; R13 binds it below.
    return {
...
def main() -> None:
    ...
    parser.add_argument("--closure-preflight", type=Path, required=True)
    ...
    closure_preflight = json.loads(args.closure_preflight.read_text())
    if (closure_preflight.get("schema") != "dynamic-vamana-w1-cp20-r13-preflight-v1"
            or closure_preflight.get("status") != "pass"):
        raise SystemExit("DiskANN R13 execution preflight is not accepted")
    bound_r10_preflight = closure_preflight.get("r10_execution_preflight_readable_copy")
    if (not isinstance(bound_r10_preflight, dict)
            or bound_r10_preflight.get("realpath") != str(args.execution_preflight.resolve())
            or bound_r10_preflight.get("sha256") != sha256(args.execution_preflight)):
        raise SystemExit("DiskANN R13/R10 preflight anti-tamper anchor mismatch")
    bound_cp20_gt = closure_preflight.get("cp20_gt")
    if (not isinstance(bound_cp20_gt, dict)
            or bound_cp20_gt.get("realpath") != str(args.gt.resolve())
            or bound_cp20_gt.get("sha256") != sha256(args.gt)):
        raise SystemExit("DiskANN R13 CP20 GT anchor mismatch")
    ...
    report = {
        "schema": "dynamic-vamana-w1-diskann-cp20-stale-r13-v1",
        "status": "pass",
        "classification": "stale-static negative control",
        "rank_with_update_throughput": False,
        "stale_result_ids_may_be_inactive_at_cp20": True,
        ...
        "identities": {
            ...
            "cp20_gt": {"realpath": str(args.gt.resolve()), "sha256": sha256(args.gt)},
            ...
            "r13_closure_preflight": standard_identity(args.closure_preflight),
        },
        "points": points,
