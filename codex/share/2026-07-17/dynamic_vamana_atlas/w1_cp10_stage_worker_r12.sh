...
usage() {
  cat >&2 <<'EOF'
usage: w1_cp10_stage_worker_r12.sh
  --mode formal --system DGAI|OdinANN --stage cp10
...
if [[ -e $result ]]; then
  [[ -d $result && -d $result/input_canary && ! -L $result/input_canary \
    ...
    echo 'stage result lacks the exact R12 input-canary evidence' >&2; exit 1;
  }
  ...
  [[ ${existing[*]} == 'input_canary input_canary/canary.json input_canary/canary.log' ]] || {
    echo 'stage result contains state beyond the R12 input canary' >&2; exit 1;
  }
else
...
# This precheck is intentionally inside the update scope.  It proves that R12
# receives only master records [400000:800000], never a cumulative prefix.
python3 - "$trace" "$delta_manifest" "${arg[expected-count]}" "${arg[delta-start]}" "$full" "$before_tags" "$after_tags" "$probes" "$probe_spec" "$local_spec" "$global_spec" "$result/worker_identity.json" "$scope" "${arg[mode]}" "${arg[system]}" "${arg[stage]}" "$attempt" "$new/w1_cp10_stage_worker_r12.sh" "${arg[forbidden-paths]}" <<'PY'
...
if (mode, stage, delta_start, count) != ("formal", "cp10", 400_000, 400_000):
    raise SystemExit("R12 worker must consume exactly master[400000:800000]")
if "master" in Path(trace).name.lower() or "replace_cp10" in Path(trace).name.lower():
    raise SystemExit("R12 worker refuses master/cumulative CP10 trace pathnames")
