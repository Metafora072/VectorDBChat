#!/usr/bin/env bash
# One shared CP00 -> CP01 -> CP05 cumulative state machine for DGAI and OdinANN.
set -euo pipefail
...
[[ ${W1_CUMULATIVE_AUTHORIZED:-0} == 1 && ${W1_GLOBAL_LOCK_HELD:-0} == 1 ]] || {
  echo 'cumulative trajectory/global-lock capability absent' >&2; exit 64;
}
...
if [[ ${arg[mode]} == formal ]]; then
  ...
  [[ ${arg[run-name]} == pilot3_sift10m_w1_cp05_trajectory && ${arg[attempt]} == trajectory-cp05-01 ]] || {
    echo 'formal run/attempt identity mismatch' >&2; exit 1;
  }
else
  [[ ${arg[cp01-count]} == 16 && ${arg[cp05-count]} == 64 ]] || {
    echo 'sequential replay counts must be 16 then 64 delta' >&2; exit 1;
  }
  [[ ${arg[run-name]} == pilot3_w1_cp05_trajectory_replay && ${arg[attempt]} == sequential-cp80-01 ]] || {
    echo 'replay run/attempt identity mismatch' >&2; exit 1;
  }
fi
...
if [[ ${arg[mode]} == formal ]]; then
  result=$(realpath -m "$root/results/pilot3_sift10m_w1_cp05_trajectory/${arg[system]}/${arg[attempt]}")
else
  result=$(realpath -m "$root/results/pilot3_sift10m_w1_cp05_trajectory/replay/${arg[system]}/${arg[attempt]}")
fi
expected_work="$root/formal/${arg[run-name]}/${arg[system]}/${arg[attempt]}"
if [[ ${arg[mode]} == formal ]]; then expected_result="$root/results/pilot3_sift10m_w1_cp05_trajectory/${arg[system]}/${arg[attempt]}"
else expected_result="$root/results/pilot3_sift10m_w1_cp05_trajectory/replay/${arg[system]}/${arg[attempt]}"; fi
...
"$old/w1_clone_base.sh" "${arg[system]}" "$base" "$work"
...
query_checkpoint() {
  ...
  for l in "${query_ls[@]}"; do
    for repetition in 1 2 3; do
      ...
      local unit="dv-w1-cum-${arg[mode]}-${system_lower}-${checkpoint}-l${l}-r${repetition}"
...
inaccessible_canary() {
  ...
  local unit="dv-w1-cum-${arg[mode]}-${system_lower}-${checkpoint}-input-canary"
...
run_stage() {
  ...
  local stage_result="$result/stages/$checkpoint" resource="$result/${checkpoint}_stage_resources.json"
  local unit="dv-w1-cum-${arg[mode]}-${system_lower}-${checkpoint}-update"
