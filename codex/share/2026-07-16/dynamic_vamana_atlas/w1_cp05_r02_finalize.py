#!/usr/bin/env python3
"""Tamper-evident finalizer for CP05 cumulative trajectory R02 outputs."""
...
SCHEMA = "dynamic-vamana-w1-cp05-cumulative-trajectory-r02-summary-v1"
RUN = "pilot3_sift10m_w1_cp05_trajectory_r02"
REPLAY_RUN = "pilot3_w1_cp05_trajectory_replay_r02"
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    old = module("w1_cumulative_finalize_r02_import", "w1_cumulative_finalize.py")
    prehelper = module("w1_cp05_r02_preflight_finalize_import", "w1_cp05_r02_preflight.py")
    ...
    require(result == root / f"results/{RUN}" and formal_root == root / f"formal/{RUN}",
            "R02 finalizer capability path mismatch")
    require(replay_result in (result / "replay", root / f"results/{REPLAY_RUN}"),
            "R02 replay result root mismatch")
    expected_report = Path(__file__).parent.parent / "dynamic_vamana_w1_cp05_cumulative_trajectory_r02_results_0716.md"
    require(args.output_report.resolve(strict=False) == expected_report.resolve(strict=False),
            "R02 report output path mismatch")
    ...
    require(preflight.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r02-preflight-v1"
            and preflight.get("status") == "pass", "R02 preflight invalid")
    preservation_path = args.preservation.resolve(strict=True); preservation = load(preservation_path)
    require(preservation.get("schema") == "dynamic-vamana-w1-cp05-cumulative-r02-preservation-v1"
            and preservation.get("status") == "pass" and not preservation.get("mismatches"),
            "R02 preservation invalid")
    ...
    for system in ("DGAI", "OdinANN"):
        replay_attempt = replay_result / system / "sequential-cp80-02"
        replay_work = replay_work_root / system / "sequential-cp80-02"
        require((replay_attempt / "CUMULATIVE_TRAJECTORY_OK").is_file(), f"{system} replay marker absent")
        old.validate_query_gate(replay_attempt, replay_work, "replay", system, "cp00")
        rcp01 = replay_attempt / "checkpoints/cp01/cp01_checkpoint_evidence.json"
        old.validate_checkpoint_chain(replay_attempt, replay_work, "replay", system, "cp01", 16, None)
        rcp05 = replay_attempt / "checkpoints/cp05/cp05_checkpoint_evidence.json"
        old.validate_checkpoint_chain(replay_attempt, replay_work, "replay", system, "cp05", 64, rcp01)
        ...
        attempt = result / system / "trajectory-cp05-02"; work = formal_root / system / "trajectory-cp05-02"
    ...
    disk_path = result / "DiskANN/stale-cp05-02/stale_control.json"; disk = old.require_pass(disk_path)
    ...
    summary = {"schema": SCHEMA, "status": "pass",
        "decision_boundary": "CP05 cumulative R02 only; CP10/CP20 remain HOLD",
    ...
    lines = ["# Dynamic Vamana W1 CP05 累计 Trajectory R02 实验结果", "", "## 裁决", "",
