#!/usr/bin/env bash
# R02-only shared CP00 -> CP01 -> CP05 state machine for DGAI and OdinANN.
set -euo pipefail
...
[[ ${W1_CP05_R02_CUMULATIVE_AUTHORIZED:-0} == 1 && ${W1_GLOBAL_LOCK_HELD:-0} == 1 ]] || {
  echo 'R02 cumulative trajectory/global-lock capability absent' >&2; exit 64;
}
...
if [[ ${arg[mode]} == formal ]]; then
  ...
  [[ ${arg[run-name]} == pilot3_sift10m_w1_cp05_trajectory_r02 && ${arg[attempt]} == trajectory-cp05-02 ]] || {
    echo 'R02 formal run/attempt identity mismatch' >&2; exit 1;
  }
else
  [[ ${arg[cp01-count]} == 16 && ${arg[cp05-count]} == 64 ]] || {
    echo 'sequential replay counts must be 16 then 64 delta' >&2; exit 1;
  }
  [[ ${arg[run-name]} == pilot3_w1_cp05_trajectory_replay_r02 && ${arg[attempt]} == sequential-cp80-02 ]] || {
    echo 'R02 replay run/attempt identity mismatch' >&2; exit 1;
  }
fi
...
[[ -f $evidence && -f $new/w1_cumulative_stage_worker.sh ]] || { echo 'cumulative helper missing' >&2; exit 1; }
...
if [[ ${arg[mode]} == formal ]]; then
  result=$(realpath -m "$root/results/pilot3_sift10m_w1_cp05_trajectory_r02/${arg[system]}/${arg[attempt]}")
else
  result=$(realpath -m "$root/results/pilot3_sift10m_w1_cp05_trajectory_r02/replay/${arg[system]}/${arg[attempt]}")
fi
expected_work="$root/formal/${arg[run-name]}/${arg[system]}/${arg[attempt]}"
if [[ ${arg[mode]} == formal ]]; then expected_result="$root/results/pilot3_sift10m_w1_cp05_trajectory_r02/${arg[system]}/${arg[attempt]}"
else expected_result="$root/results/pilot3_sift10m_w1_cp05_trajectory_r02/replay/${arg[system]}/${arg[attempt]}"; fi
...
[[ $base == "$expected_base" ]] || { echo "R02 base identity mismatch: $base" >&2; exit 1; }
...
export ATLAS_W1_MUTABLE_CHAT="$new"
if [[ ${arg[mode]} == replay ]]; then
  clone_helper="$new/w1_clone_replay_base_r02.sh"
else
...
install -d -o ubuntu -g ubuntu "$result/queries" "$result/stages" "$result/checkpoints"
...
mode_manifest() {
  python3 "$new/w1_mode_manifest.py" write --root "$1" --output "$2"
}
assert_base_exact() {
...
query_checkpoint() {
  ...
  for l in "${query_ls[@]}"; do
    for repetition in 1 2 3; do
      ...
      local unit="dv-w1-cum-r02-${arg[mode]}-${system_lower}-${checkpoint}-l${l}-r${repetition}"
      systemd-run --scope --collect --unit "$unit" --uid ubuntu \
        ...
        --property=IOAccounting=yes --property="MemoryMax=$query_memory" \
        ...
        prlimit --core=0:0 -- numactl --physcpubind=0-23 --membind=0 \
          ...
          "$old/w1_query_worker.sh" "${arg[system]}" "$query_binary" "$work/index/index" \
            "$query" "$gt" "$stem.result_ids.bin" "$stem.log" "$l" "$active"
    done
  done
...
inaccessible_canary() {
  ...
  local output="$result/${checkpoint}_inaccessible_canary.json"
  local unit="dv-w1-cum-r02-${arg[mode]}-${system_lower}-${checkpoint}-input-canary"
  systemd-run --wait --collect --pipe --unit "$unit" --uid ubuntu --property=Type=exec \
    ...
    env -i PATH=/usr/bin:/bin LANG=C LC_ALL=C PYTHONDONTWRITEBYTECODE=1 \
      python3 -c 'import errno,json,os,sys; allowed,out,*denied=sys.argv[1:]; fd=os.open(allowed,os.O_RDONLY|os.O_NOFOLLOW); os.close(fd); rows=[]
for path in denied:
 try: fd=os.open(path,os.O_RDONLY|os.O_NOFOLLOW)
 except OSError as exc:
  if exc.errno not in (errno.EACCES,errno.EPERM): raise
  rows.append({"path":path,"open_refused":True,"errno":exc.errno})
 else: os.close(fd); raise SystemExit("forbidden input readable: "+path)
open(out,"x").write(json.dumps({"schema":"dynamic-vamana-w1-inaccessible-input-canary-v1","status":"pass","allowed_delta":allowed,"denied":rows},indent=2)+"\n")' \
      "$allowed" "$output" "${denied[@]}" >>"$result/${checkpoint}_inaccessible_canary.log" 2>&1
...
PY
  ...
  python3 - "$result/${checkpoint}_controller.log" "$result/${checkpoint}_inaccessible_canary.json" <<'PY'
import json,re,sys
...
PY
  local -a evidence_args=(runuser -u ubuntu -- env PYTHONDONTWRITEBYTECODE=1 python3 "$evidence" stage-evidence
    ...
    --input-capability-canary "$result/${checkpoint}_inaccessible_canary.json"
...
run_stage() {
  ...
  local stage_result="$result/stages/$checkpoint" resource="$result/${checkpoint}_stage_resources.json"
  local unit="dv-w1-cum-r02-${arg[mode]}-${system_lower}-${checkpoint}-update"
  local inaccessible_key="${checkpoint}-inaccessible" inaccessible_csv
  inaccessible_csv=${arg[$inaccessible_key]}
  [[ ! -e $stage_result && ! -e $resource ]] || { echo "$checkpoint stage freshness failed" >&2; exit 1; }
  stage_health_guard
  ...
  systemd-run --wait --collect --pipe --unit "$unit" --uid ubuntu --property=Type=exec \
    ...
    numactl --physcpubind=0-23 --membind=0 \
      ...
      python3 "$old/resource_probe.py" --output "$resource" --interval-ms 25 --space-root "$work/index" \
        --space-interval-seconds 1 -- \
      "$new/w1_cumulative_stage_worker.sh" --mode "${arg[mode]}" --system "${arg[system]}" --stage "$checkpoint" \
