#!/usr/bin/env python3
"""Identity-v2 validation for DiskANN CP00-index versus CP05-GT stale control."""
...
def validate_accepted_lineage(args: argparse.Namespace, artifact: dict[str, Any]) -> dict[str, object]:
    ...
    if preflight.get("status") != "pass" or preflight.get("schema") != "dynamic-vamana-w1-cp05-cumulative-preflight-v1":
        raise ValueError("CP05 execution preflight is not accepted")
...
def main() -> None:
    ...
    report = {
        "schema": "dynamic-vamana-w1-diskann-cp05-stale-v1",
        "status": "pass",
