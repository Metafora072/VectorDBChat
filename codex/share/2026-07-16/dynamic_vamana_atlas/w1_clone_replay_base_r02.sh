#!/usr/bin/env bash
# Clone one exact R02 immutable replay base into its exact mutable attempt.
set -euo pipefail

[[ ${W1_CP05_R02_CUMULATIVE_AUTHORIZED:-0} == 1 && ${W1_FORMAL_PATH_AUTHORIZED:-0} == 1 ]] || { echo 'R02 replay clone gate not granted' >&2; exit 64; }
...
mutable_chat=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
chat=${ATLAS_W1_V1_CHAT:-/home/ubuntu/pz/VectorDB/chat/codex/share/2026-07-15/dynamic_vamana_atlas}
...
for name in W1_ALLOWED_CLONE_TARGET W1_ALLOWED_CLONE_SYSTEM W1_ALLOWED_CLONE_RUN W1_ALLOWED_CLONE_ATTEMPT; do
  [[ -n ${!name:-} ]] || { echo "R02 replay clone capability missing: $name" >&2; exit 2; }
done
expected_run=pilot3_w1_cp05_trajectory_replay_r02
expected_attempt=sequential-cp80-02
...
[[ $W1_ALLOWED_CLONE_RUN == "$expected_run" && $W1_ALLOWED_CLONE_ATTEMPT == "$expected_attempt" \
  ...
  echo 'R02 replay clone identity mismatch' >&2; exit 2;
}
[[ -f "$(dirname "$base")/IMMUTABLE_REPLAY_BASE_OK" ]] || { echo 'R02 immutable replay marker absent' >&2; exit 1; }
python3 "$mutable_chat/w1_replay_base_recovery.py" verify --root "$root" --system "$system"
...
python3 "$mutable_chat/w1_mode_manifest.py" write --root "$base" --output "$tmp/base_mode_before.tsv"
python3 "$mutable_chat/w1_mode_manifest.py" write --root "$base" --output "$tmp/base_mode_after_clone.tsv"
cmp -s "$tmp/base_mode_before.tsv" "$tmp/base_mode_after_clone.tsv" || { echo 'base mode changed during clone' >&2; exit 1; }
python3 "$mutable_chat/w1_mode_manifest.py" write --root "$tmp/index" --output "$tmp/clone_mode_before.tsv"
python3 "$mutable_chat/w1_mode_manifest.py" compare --left "$tmp/base_mode_before.tsv" --right "$tmp/clone_mode_before.tsv" --policy-only
...
python3 "$mutable_chat/w1_prepare_mutable_clone.py" --clone-root "$tmp/index" --base-root "$base" --owner "$owner" \
...
python3 "$mutable_chat/w1_mode_manifest.py" write --root "$tmp/index" --output "$tmp/clone_mode_after.tsv"
python3 "$mutable_chat/w1_mode_manifest.py" verify-private --manifest "$tmp/clone_mode_after.tsv" --owner "$owner"

audit_cmd=(python3 "$mutable_chat/w1_writable_clone_audit.py" --clone-root "$tmp/index" --base-root "$base" --owner "$owner" --output "$tmp/mutable_clone_audit.json")
...
python3 "$mutable_chat/w1_mode_manifest.py" write --root "$base" --output "$tmp/base_mode_after_audit.tsv"
...
python3 "$mutable_chat/w1_mode_manifest.py" write --root "$target/index" --output "$target/final_mode.tsv"
cmp -s "$target/clone_mode_after.tsv" "$target/final_mode.tsv" || { echo 'published clone mode mismatch' >&2; exit 1; }
python3 "$mutable_chat/w1_mode_manifest.py" verify-private --manifest "$target/final_mode.tsv" --owner "$owner"
...
python3 "$mutable_chat/w1_mode_manifest.py" write --root "$base" --output "$target/base_mode_after_publish.tsv"
