...
def main() -> None:
    ...
    if a.output.exists(): raise SystemExit("R03 reuse preservation report overwrite refused")
    ...
    a.output.write_text(json.dumps({"schema": "dynamic-vamana-w1-r03-reuse-preservation-v1",
        "status": "pass" if not mismatches else "fail", "cp01_unchanged": not any(row["kind"].startswith("cp01") for row in mismatches),
        "r02_gt_unchanged": not any(row["kind"] == "r02_gt" for row in mismatches), "r02_gt_sha256": gt_sha,
        "cp01_artifacts": current, "mismatches": mismatches}, indent=2) + "\n")
    if mismatches: raise SystemExit("R03 reused-input preservation failed")
