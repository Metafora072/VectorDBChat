#!/usr/bin/env python3
"""Create or transition the fail-closed CP05 cumulative R02 execution manifest."""
...
def main() -> None:
    ...
    if args.action == "activate":
        ...
        if args.manifest.exists() or any(path is None or not path.is_file() for path in required):
            raise SystemExit("R02 activation requires fresh manifest and all frozen gate evidence")
        preflight = json.loads(args.preflight.read_text())
        if preflight.get("status") != "pass":
            raise SystemExit("preflight did not pass")
        old = json.loads(args.old_attempt_manifest.read_text())
        if old.get("status") != "stopped_failed" or old.get("stopped_phase") != "replay_DGAI":
        ...
        payload: dict[str, object] = {
            "schema": "dynamic-vamana-w1-cp05-cumulative-r02-execution-v1",
            "status": "running",
            ...
            "formal_root": str(atlas_root / "formal/pilot3_sift10m_w1_cp05_trajectory_r02"),
            "replay_run": "pilot3_w1_cp05_trajectory_replay_r02",
            "replay_formal_root": str(atlas_root / "formal/pilot3_w1_cp05_trajectory_replay_r02"),
            "execution_delta_root": str(atlas_root / "datasets/sift10m/w1_trajectory/execution_deltas_r02"),
            ...
            "terminal_r01_manifest_realpath": str(args.old_attempt_manifest.resolve()),
            "terminal_r01_manifest_sha256": sha256(args.old_attempt_manifest),
            "authorized_sequence": ["CP00", "CP01", "CP05"],
            "attempts": {
                "DGAI_replay": "sequential-cp80-02",
                "OdinANN_replay": "sequential-cp80-02",
                "DGAI_formal": "trajectory-cp05-02",
                "OdinANN_formal": "trajectory-cp05-02",
                "DiskANN": "stale-cp05-02",
            },
            "old_attempt_reused": False,
            "old_delta_reused": False,
            "cp10_cp20": "HOLD",
            "retry_policy": "none",
        }
    else:
