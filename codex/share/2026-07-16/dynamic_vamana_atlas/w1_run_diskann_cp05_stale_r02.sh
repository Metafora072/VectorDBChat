#!/usr/bin/env bash
# R02-only frozen DiskANN CP00 index queried against exact CP05 GT.
set -euo pipefail
export PYTHONDONTWRITEBYTECODE=1
[[ ${W1_CP05_R02_CUMULATIVE_AUTHORIZED:-0} == 1 && ${W1_GLOBAL_LOCK_HELD:-0} == 1 ]] || {
  echo 'CP05 cumulative R02 authorization/global lock absent' >&2; exit 64;
}
...
expected_out="$root/results/pilot3_sift10m_w1_cp05_trajectory_r02/DiskANN/stale-cp05-02"
expected_gt="$root/groundtruth/sift10m/w1_trajectory/cp05/gt_cp05"
expected_query="$root/datasets/sift10m/query.bin"
expected_preflight="$root/results/pilot3_sift10m_w1_cp05_trajectory_r02/preflight/execution_preflight.json"
...
python3 "$new/w1_diskann_cp05_validate_r02.py" --result-dir "$out" --binary "$binary" \
