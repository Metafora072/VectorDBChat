...
def main() -> None:
    ...
    p.add_argument("--run-label", choices=("R03", "R04"), default="R03")
    ...
    lines = [f"# Dynamic Vamana W1 {a.run_label} Continuation 停止报告", "", "## 结论", "",
             f"{a.run_label} 在 `{a.phase}` 阶段以退出码 `{a.exit_code}` fail closed。execution manifest 状态为 `{manifest.get('status', 'unavailable')}`，后续系统未由 continuation controller 启动。本报告不支持任何尚未完整通过的 1% churn 结论。", "",
             "## 执行边界", "", f"{a.run_label} 仅复用已验证的 R02 GT 与 CP01，从系统阶段开始。控制器保留已经生成的 attempt、结果、资源和日志，不自动重试、不改参数，也不续写旧轮次。停止后的 reused-input preservation 状态为 `{preservation}`。", "",
             "## 证据", "", f"停止时间为 `{datetime.datetime.now(datetime.timezone.utc).isoformat()}`。当前 {a.run_label} result tree 中共有 `{len(evidence)}` 个证据文件，根目录为 `{a.result}`。具体失败位置以 `execution_manifest.json`、controller log 和阶段日志为准。", ""]
