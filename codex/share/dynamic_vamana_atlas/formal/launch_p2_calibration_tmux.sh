...
umask 077
{
  ...
  # events-canary-01 is retained as a known failed diagnostic attempt; reuse
  # the already-passed event-02 canary when resuming P2.
  printf 'export P2_CANARY_ATTEMPT=%q\n' "${P2_CANARY_ATTEMPT:-events-canary-02}"
} >"$ENV_FILE"
umask 022
