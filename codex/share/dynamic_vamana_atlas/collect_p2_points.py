...
def main() -> None:
    ...
    for target in TARGETS:
        ...
        for system in SYSTEMS:
            recalls = sorted(row["recall_at_10"] for row in rows if row["system"] == system and row["query_threads"] == 1)
    ...
    args.summary.write_text(json.dumps({"schema": "dynamic-vamana-p2-calibration-summary-v1", "targets": coverage,
                                        "common_targets": common, "point_count": len(rows)}, indent=2) + "\n")
