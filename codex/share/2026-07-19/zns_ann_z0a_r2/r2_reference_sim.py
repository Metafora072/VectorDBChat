...
class Reference:
    ...
    def truncate(self, event: dict[str, Any], action: dict[str, Any]) -> None:
        keys = [str(key) for key in event.get("invalidated_keys", [])]
        if len(keys) != len(set(keys)):
            raise RefFault("DUPLICATE_TRUNCATE_KEY")
        for key in keys:
            old = self.locations().get(key)
            if old is None:
                raise RefFault("TRUNCATE_MISSING_KEY", key)
            self.z[old[1]]["slots"][old[2]][2] = False
