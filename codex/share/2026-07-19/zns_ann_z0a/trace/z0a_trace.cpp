...
class Collector {
 public:
  Collector() {
    const char *enabled = std::getenv("ATLAS_Z0A_ENABLED");
    enabled_ = enabled && std::strcmp(enabled, "1") == 0;
    if (!enabled_) return;
    ...
    if (trace_output_.empty() || meta_output_.empty() || ledger_output_.empty() || index_root_.empty() ||
  ...
  bool enabled() const { return enabled_ && !configuration_error_ && records_; }
  ...
  bool submit(AtlasZ0ATraceToken *token, uint16_t source, int fd, uint64_t offset, uint64_t length) {
    ...
    record.submit_seq = submit_seq_.fetch_add(1, std::memory_order_relaxed) + 1;
  ...
  void complete(AtlasZ0ATraceToken *token, int64_t result, int64_t status) {
    if (!enabled() || !token || token->guard != kTokenGuard || token->slot >= capacity_) return;
  ...
  void flush() {
    ...
    write_meta(count, trace_ok);
    write_ledger(count, trace_ok);
  }
 ...
 private:
  ...
  void write_meta(uint64_t count, bool trace_ok) {
    std::ostringstream out;
    out << "{\n  \"schema\": \"zns-ann-z0a-trace-meta-v1\",\n  \"status\": \""
        ...
        << ",\n  \"buffer_peak_bytes\": " << buffer_bytes_ << ",\n  \"dropped_events\": " << dropped_.load()
  ...
  void write_ledger(uint64_t count, bool trace_ok) {
  ...
  std::string trace_output_, meta_output_, ledger_output_, index_root_, system_, run_id_, object_map_;
  uint64_t run_hash_ = 0, capacity_ = 0, buffer_bytes_ = 0;
  TraceRecord *records_ = nullptr;
  std::atomic<uint64_t> reserved_{0}, dropped_{0}, request_id_{0}, submit_seq_{0}, completion_seq_{0}, identity_errors_{0};
