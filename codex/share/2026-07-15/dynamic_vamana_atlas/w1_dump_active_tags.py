...
def main()->None:
 p=argparse.ArgumentParser();p.add_argument('--tags',type=Path,required=True);p.add_argument('--expected',type=Path,required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args()
 ...
 report['valid']=report['active_tag_count']==8_000_000 and duplicate==0 and report['expected_exact_match']
