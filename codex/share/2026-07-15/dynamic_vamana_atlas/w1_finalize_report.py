...
def query_rows(result_root: Path) -> list[dict]:
    rows = []
    for system in ("DGAI", "OdinANN"):
        directory = result_root / system / "cp01-01"
...
def main() -> None:
    ...
    a = p.parse_args()
    result_root = a.root / "results/pilot3_sift10m_w1"
    required = [result_root / "preflight/execution_preflight.json", result_root / "preparation/cp01_preparation_resources.json",
                result_root / "preparation/gt_cp01_resources.json", result_root / "DGAI/cp01-01/FORMAL_W1_CANARY_OK",
                result_root / "OdinANN/cp01-01/FORMAL_W1_CANARY_OK", result_root / "DiskANN/stale-cp00-01/DISKANN_STALE_CONTROL_OK"]
    missing = [str(path) for path in required if not path.exists()]
    if missing:
        raise SystemExit(f"formal report prerequisites missing: {missing}")
    prep = resources(required[1]); gt = resources(required[2]); queries = query_rows(result_root)
    canaries = {system: load(result_root / system / "cp01-01/canary.json") for system in ("DGAI", "OdinANN")}
    gates = {system: load(result_root / system / "cp01-01/preupdate_gate.json") for system in ("DGAI", "OdinANN")}
    stale = load(result_root / "DiskANN/stale-cp00-01/stale_control.json")
    ...
    (raw / "artifact_index.json").write_text(json.dumps({"schema": "dynamic-vamana-w1-artifact-index-v1",
        "dynamic_system_results": {system: str((result_root / system / 'cp01-01').resolve()) for system in canaries},
        "diskann_control": str((result_root / 'DiskANN/stale-cp00-01').resolve()),
    ...
    lines = ["# Dynamic Vamana W1 1% Replace-New Canary 实验结果", "",
             ...
             "## 方法与执行边界", "", "实验在项目 NVMe 上持有单一 global flock 串行执行。fresh execution preflight 通过后，依次完成 CP01 trace、独立验证、active-vector materialization、exact GT、DGAI、OdinANN 与 DiskANN stale-static control。正式 trace 使用 seed `20260713`，包含 80,000 个唯一删除和 80,000 个唯一插入，最终 active cardinality 为 8,000,000。DGAI ingestion 仅覆盖原生 insert/delete API，trace 解析、clone、load、publish 与 probe 均排除在该区间之外。", "",
             "## Preparation 与 GT 资源", "", "| 阶段 | Wall time(s) | Peak RSS(B) | cgroup memory peak(B) | NVMe read(B) | NVMe write(B) | Peak allocated(B) |", "|---|---:|---:|---:|---:|---:|---:|",
             f"| CP01 preparation | {fmt(prep['elapsed_seconds'])} | {prep['peak_rss_bytes']} | {prep['cgroup_memory_peak_bytes']} | {prep['nvme_read_bytes']} | {prep['nvme_write_bytes']} | {prep['peak_allocated_bytes']} |",
             f"| checkpoint-1 exact GT | {fmt(gt['elapsed_seconds'])} | {gt['peak_rss_bytes']} | {gt['cgroup_memory_peak_bytes']} | {gt['nvme_read_bytes']} | {gt['nvme_write_bytes']} | {gt['peak_allocated_bytes']} |", "",
    ...
    for system, data in canaries.items():
        ...
        online_path = result_root / system / "cp01-01/online_probe.json"
    ...
    for system, data in canaries.items():
        ...
        update_res = resources(result_root / system / "cp01-01/resources.json")
