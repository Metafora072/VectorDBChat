...
def digest(path):
 ...
 return h.hexdigest()
ds,out=map(Path,sys.argv[1:]); manifest=json.loads((ds/'replace_cp01_manifest.json').read_text())
manifest.update({'active_vector_sha256':digest(ds/'active_cp01.bin'),'active_tag_sha256':digest(ds/'active_cp01.tags.bin'),'query_sha256':digest(Path('/home/ubuntu/pz/VectorDB/data/VectorDB/dynamic_vamana_atlas/datasets/sift10m/query.bin')),'truthset_sha256':digest(out/'gt_cp01'),'truthset_shape':'10000x100'})
