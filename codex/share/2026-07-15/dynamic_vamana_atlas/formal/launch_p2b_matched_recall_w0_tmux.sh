...
{ printf 'export ATLAS_ROOT=%q\n' "$ROOT"; printf 'export ATLAS_CHAT_ROOT=%q\n' "$CHAT"; printf 'export ATLAS_RUN_NAME=%q\n' "$RUN_NAME"; printf 'export ATLAS_OPERATOR_USER=%q\n' "${ATLAS_OPERATOR_USER:-ubuntu}"; printf 'export ATLAS_NOTIFY_EMAIL=%q\n' "${ATLAS_NOTIFY_EMAIL:-1}"; } >"$ENV_FILE"
umask 022; printf -v command 'set -a; source %q; exec %q' "$ENV_FILE" "$CHAT/formal/run_p2b_matched_recall_w0.sh"
tmux new-session -d -s "$SESSION" "$command"; echo "started tmux:$SESSION"
