#!/usr/bin/env bash
set -euo pipefail
[[ ${M1_WRITE_ATTRIBUTION_AUTHORIZED:-0} == 1 && ${M1_GLOBAL_LOCK_HELD:-0} == 1 ]] || { echo 'M1 authorization/global lock absent' >&2; exit 64; }
(( EUID == 0 )) || { echo 'M1 runner must execute as root' >&2; exit 1; }
[[ $# == 2 && ( $1 == DGAI || $1 == OdinANN ) && ( $2 == 50000 || $2 == 200000 || $2 == 400000 ) ]] || { echo 'usage: m1_run_one.sh DGAI|OdinANN 50000|200000|400000' >&2; exit 2; }
system=$1; size=$2
...
run=pilot3_sift10m_write_attribution_m1_scale_r01; attempt="m1-n${size}-01"
work="$root/formal/$run/$system/$attempt"; result="$root/results/$run/$system/$attempt"; input="$root/results/$run/inputs/n${size}"
build="$root/build/write-attribution-m1-v5-r01"; driver="$build/install/$system/w1_canary"; canonical="$root/build/w1-canonical-v6/install/$system/w1_canary"
...
for path in "$build/M1_V5_BUILD_OK" "$build/build_manifest.json" "$driver" "$canonical" "$full" "$trace" "$expected" "$probes" "$probe_spec" "$freeze"; do
  [[ -f $path ]] || { echo "missing M1 input: $path" >&2; exit 1; }
done
[[ ! -e $work && ! -e $result ]] || { echo "refusing M1 attempt reuse: $system/$attempt" >&2; exit 1; }
...
profile="$result/app_write_profile_v5.json"; markers="$result/markers.jsonl"; resources="$result/resources.json"; primer="$result/io_primer.json"; online="$result/online.bin"
...
unit="dv-m1-r01-${system_lower}-n${size}-01"
...
for path in "$profile" "$markers" "$resources" "$primer"; do [[ -s $path ]] || { echo "missing M1 evidence: $path" >&2; exit 1; }; done
...
touch "$result/M1_V5_RUN_OK"; chmod -R a-w "$result"; echo "$result/summary.json"
