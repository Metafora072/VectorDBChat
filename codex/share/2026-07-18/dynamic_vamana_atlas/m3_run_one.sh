#!/usr/bin/env bash
set -euo pipefail
[[ ${M3_WRITE_SUPERSESSION_AUTHORIZED:-0} == 1 && ${M3_GLOBAL_LOCK_HELD:-0} == 1 ]] || { echo 'M3 authorization/global lock absent' >&2; exit 64; }
(( EUID == 0 )) || { echo 'M3 runner must execute as root' >&2; exit 1; }
...
run=pilot3_sift10m_write_supersession_m3_r01; attempt="m3-n${size}-01"
...
build="$root/build/write-supersession-m3-v1-r01"; driver="$build/install/$system/w1_canary"; canonical="$root/build/w1-canonical-v6/install/$system/w1_canary"
...
for path in "$build/M3_BUILD_OK" "$build/build_manifest.json" "$driver" "$canonical" "$full" "$trace" "$expected" "$probes" "$probe_spec" "$freeze"; do
  [[ -f $path ]] || { echo "missing M3 input: $path" >&2; exit 1; }
done
[[ ! -e $work && ! -e $result ]] || { echo "refusing M3 attempt reuse: $system/$attempt" >&2; exit 1; }
...
profile="$result/app_write_profile_v5.json"; logical="$result/neighbor_repair_logical.json"; lifecycle="$result/write_lifecycle.json"; markers="$result/markers.jsonl"; resources="$result/resources.json"; primer="$result/io_primer.json"; online="$result/online.bin"
libs="$build/lib:$root/build/gperftools-install/lib:$root/build/openblas-install/lib:$root/build/jemalloc-install/lib"
driver_args=(env LD_PRELOAD="$build/lib/libm0write.so" ATLAS_W1_MARKERS="$markers" ATLAS_M0_INDEX_ROOT="$work/index" ATLAS_M0_PROFILE_OUTPUT="$profile" ATLAS_M2_LOGICAL_OUTPUT="$logical" ATLAS_M3_LIFECYCLE_OUTPUT="$lifecycle" "$driver" run "$full" "$prefix" "$trace")
online_args=()
if [[ $system == OdinANN ]]; then driver_args+=("$probes" "$online"); fi
unit="dv-m3-r01-${system_lower}-n${size}-01"
...
for path in "$profile" "$logical" "$lifecycle" "$markers" "$resources" "$primer"; do [[ -s $path ]] || { echo "missing M3 evidence: $path" >&2; exit 1; }; done
...
python3 "$chat/m3_validate.py" --system "$system" --size "$size" --physical-summary "$result/physical_summary.json" --m2-logical "$logical" --m2-run-summary "$result/m2_summary.json" --lifecycle "$lifecycle" --m2-baseline "$root/results/pilot3_sift10m_neighbor_repair_m2_r01/m2_summary.json" --output "$result/m3_summary.json"
touch "$result/M3_RUN_OK"; chmod -R a-w "$result"; echo "$result/m3_summary.json"
