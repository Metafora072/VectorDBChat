...
- Gate:
  ...
  5. repeat p50 drift ≤25% for every system/L;
...
- Execution: all L values in one loaded process per representation; one recall run and three warm-up-controlled performance repetitions. Returned IDs and raw per-query counters are retained.
...
| M2 | Canary | 3 systems × 4 L × 2 repeats | all six Canary gates | 0.3–0.75 h | graph oracle too weak |
| M3 | Full matrix | 4 systems × 5 L × 3 repeats | 60 complete performance points | 1.5–4 h | high-L exact CPU/I/O |
| M4 | analysis/report | deterministic aggregation | B4 decision only | 0.5–1 h | interpolation overclaim |

Hard wall: **10 wall-clock hours after data are locally frozen**. Stop rather than reduce query count, remove Exact, change L after seeing results, or run mixed precision.
...
- NVMe:
  ...
  - Current writable filesystem has ~139 GB free.
- Time:
  - Cohere path: expected **4–9 h**, hard wall 10 h after freeze; download time reported separately.
  - GIST fallback: expected **3–6 h**. Prior local 800K GIST build took ~3,059 s end-to-end, including ~2,854 s quantized-data generation, providing the calibration.
- Raw artifacts: base vectors and indexes stay outside Git; manifests, compressed per-query CSVs, summaries, logs, commands, and figures enter the dated work directory.
