#!/usr/bin/env python3
"""Summarize one high-dimensional multi-L process."""
...
search_ls = sorted({int(row["L"]) for row in rows})
if len(search_ls) not in (4, 5) or len(rows) % len(search_ls):
    raise ValueError(f"incomplete L blocks: rows={len(rows)}, L={search_ls}")
...
for search_l in search_ls:
    ...
    nav_bytes = args.dimension * 4.0 * mean_exact
    out.append({
        "phase": args.phase,
        "label": args.label,
        "mode": args.mode,
